...
>
> **Kaynak sınırı notu (#2453)**: çoklu konteyner kurulumu varsayılan olarak `hermes-agent-src` dizinini WebUI'ye salt okunur bağlar. Bu, WebUI tarafında kaynak yeniden yazımını engeller ancak yine de bir uygulama-bağlaşım köprüsüdür, kararlı bir Agent API sınırı değildir. Mevcut kaynak/API ayrıştırma envanteri için [`docs/rfcs/agent-source-boundary.md`](docs/rfcs/agent-source-boundary.md) belgesine bakın.
...
### Topluluk saha raporu: AVF üzerinden ARM64 Android

[#2364](https://github.com/nesquena/hermes-webui/issues/2364) numaralı topluluk raporu,
Hermes Agent + WebUI'nin Android Virtualization Framework (AVF) üzerinden
bir Debian 12 VM içinde, orta seviye bir ARM64 Android telefonda çalıştığını belgelemektedir.
Bildirilen kurulumda Xiaomi Redmi Note 13 Pro 4G, VM'ye 3.8 GiB RAM ayrılmış, 8 görünür
CPU çekirdeği, Android'de Chrome ile `localhost:8787` ve bulut tabanlı çıkarım kullanılmıştır.

Bu resmi bir destek temeli veya sağlayıcı/model kıyaslaması değildir, ancak mobil ARM64
denemeleri için faydalı bir uyumluluk sinyalidir: WebUI Chrome'da akıcı şekilde görüntülendi,
ARM64 Debian ajan yığını için çalıştı ve toplam yerel ayak izi yaklaşık 1.7 GB oldu.
Rapordan pratik uyarılar: bağımlılıklar kaynaktan derlenirken ilk kurulum daha uzun sürebilir,
Android tarayıcı sekmeleri uygulamalar arası geçişte yeniden yüklenebilir ve daha uzun süreli
oturumlar için terminal veya VM ana bilgisayarı için pil optimizasyonunu devre dışı bırakmak
gerekebilir.
...
- Dosya ekleri sayfa yeniden yüklemelerinde kalıcıdır ve varsayılan olarak aktif çalışma alanı dışında saklanır (`~/.hermes/webui/attachments/<session_id>/` veya yapılandırıldığında `HERMES_WEBUI_ATTACHMENT_DIR/<session_id>/`)
...
### Temalar
- Görünüm iki eksene ayrılmıştır: Tema (`system`, `dark`, `light`) ve Cilt
  (`default`, `ares`, `mono`, `slate`, `poseidon`, `sisyphus`, `charizard`,
  `sienna`, `catppuccin`, `nous`)
- Ayarlar -> Görünüm üzerinden değiştirin (anlık canlı önizleme) veya `/theme <tema-veya-cilt>`
- Yeniden yüklemelerde kalıcıdır (settings.json'da sunucu tarafı + titreşimsiz yükleme için localStorage)
- Ciltler `data-skin` ve CSS değişkenleri kullanır; koyu mod `.dark` sınıfı üzerinden çözümlenir,
  bir `data-theme` özel tema ekseni değil — bkz. [THEMES.md](THEMES.md)
...
Bakımı yapanlar ve araçlar için:
- `CHANGELOG.md` — sürüm notları
- `SPRINTS.md` — ileriye dönük sprint planı
- `THEMES.md` — tema sistemi dokümantasyonu, özel tema kılavuzu
- `docs/CONTRACTS.md` — katkıda bulunanlar ve ajanlar için proje sözleşme/RFC/tasarım dizini
- `docs/UIUX-GUIDE.md` — mevcut tasarım belgelerinden ve görsel envanterlerden derlenen UI/UX prensipleri
- `docs/docker.md` — Docker compose kurulumu, yaygın hatalar ve bind-mount geçişi

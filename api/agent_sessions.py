...
def open_state_db_readonly(db_path: Path, log: logging.Logger | None = None) -> sqlite3.Connection:
    ...
    avoids that. There is deliberately no writable fallback: this helper is
    used by pure-read projections, and silently upgrading a failed read into a
    writer can race the gateway and delete/replace its WAL files.

    The caller must ensure ``db_path`` exists — this raises ``FileNotFoundError``
    for a missing path rather than creating an empty database in the agent's
    home. Read-only open errors are propagated to the caller, which should
    degrade the optional projection to an empty result.
    ...
    try:
        ...
        log.warning("agent state.db read-only open failed for %s: %s", db_path, exc)
        raise
...
def read_importable_agent_session_rows(
    ...
    # The projection is strictly read-only. If the DB is busy, damaged, or
    # unavailable, the optional agent-session projection degrades to no rows;
    # it must never reopen the live WAL database as a writer.
    try:
    ...
    with closing(conn):
        ...
        # Defensive index check (#3887). The normal candidate-ordering shape uses
        ...
        # This projection is strictly read-only, so a missing index uses the
        # pre-aggregated cron-only path below rather than mutating state.db.

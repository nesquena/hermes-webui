...
def _session_recovery_memory_advisory_receipt() -> dict:
    """Return the fixed no-authority Memory Tree advisory envelope.

    This receipt is generated by the recovery boundary itself and intentionally
    does not accept caller input. It mirrors the public Capy Spaces advisory
    envelope shape so downstream context surfaces can prove any Memory Tree
    evidence remains metadata-only and cannot bypass recovery safety gates.
    """
    return {
        "metadata_only": True,

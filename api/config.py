...
def resolve_custom_provider_connection(provider_id: str) -> tuple[str | None, str | None]:
    ...
    # The provider_id slug part is already canonical (generated by
    # _custom_provider_slug_from_name).  Extract it directly.
    raw_slug = pid.split(":", 1)[1].strip()
    if not raw_slug:
        return None, None
    # Also compute the old ASCII-projection slug for backward compat
    # with sessions persisted before the canonical slug system.
    old_slug = _slugify(raw_slug)
    ...
    for entry in custom_providers:
        ...
        # Backward compat: old sessions may have persisted the ASCII-projection
        # slug (e.g. "custom:proxy" for "我的Proxy服务器").
        entry_slug = _slugify(name)
        if entry_slug and entry_slug == old_slug:
            base_url = str(entry.get("base_url") or "").strip() or None
            api_key = _resolve_key(entry.get("api_key"), entry.get("key_env"), pid)
            return api_key, base_url
    ...
    if not fallback_key and isinstance(model_cfg, dict) and model_provider in {"custom", pid, raw_slug, old_slug}:

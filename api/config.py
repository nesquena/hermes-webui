...
def resolve_custom_provider_connection(provider_id: str) -> tuple[str | None, str | None]:
    ...
    for entry in custom_providers:
        ...
        # Also match by raw lowered name (for non-slugified CJK names).
        if name.lower() == raw_slug:
            base_url = str(entry.get("base_url") or "").strip() or None
            api_key = _resolve_key(entry.get("api_key"), entry.get("key_env"), pid)
            return api_key, base_url
    ...
    if not fallback_key and isinstance(model_cfg, dict) and model_provider in {"custom", pid, raw_slug}:

...
def _streams_lock_health(timeout_seconds: float = 0.5) -> dict:
    # Use live config objects rather than module-level aliases. Some reload/test
    # paths replace api.config.STREAMS while api.routes remains imported; health
    # should report the currently authoritative stream registry.
    from api import config as _live_config

    streams = getattr(_live_config, "STREAMS", STREAMS)
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/spaces/revision/restore-widget":
        ...
        try:
            ...
            return bad(handler, "Revision or widget not found", 404)
    ...
    if parsed.path == "/api/spaces/widget/event":
        ...
        try:
            ...
            if not space_id or not widget_id:
                return bad(handler, "Missing space_id or widget_id")
            event_name_values = []
            for alias in ("event_name", "eventName"):
                ...
                if alias_value:
                    event_name_values.append(alias_value)
            if event_name_values and any(value != event_name_values[0] for value in event_name_values[1:]):
                raise ValueError("Conflicting widget event name aliases")
            event_name = event_name_values[0] if event_name_values else "agent.prompt"

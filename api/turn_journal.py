...
def read_turn_journal_event_bounded(
    session_id: str,
    *,
    turn_id: str,
    stream_id: str,
    ...
    """Read back one newly appended event from this process's shard.

    Ordinary turn IDs are generated by ``append_turn_journal_event`` and the
    append lands at the tail of the current PID shard. Reading only a bounded
    tail proves the exact durable write without making chat admission scale with
    the complete lifetime journal. An event larger than the bound fails closed.
    """
    expected_turn = str(turn_id or "").strip()
    expected_stream = str(stream_id or "").strip()
    if not expected_turn or not expected_stream:
        raise ValueError("turn_id and stream_id are required")
    try:
        bound = int(max_bytes)
    except (TypeError, ValueError) as exc:
        raise ValueError("max_bytes must be an integer") from exc
    if bound < 1024 or bound > 16 * 1024 * 1024:
    ...
    for raw in payload.splitlines():
        ...
        if not isinstance(event, dict):
            continue
        if (
            str(event.get("turn_id") or "") == expected_turn
            and str(event.get("stream_id") or "") == expected_stream
        ):
            matches.append(event)
    if len(matches) > 1:
        raise RuntimeError("duplicate durable turn-journal event")

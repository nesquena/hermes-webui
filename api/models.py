...
class Session:
    ...
    def _save_owned_generation(
        ...
        try:
            if self.path.exists():
                ...
                # Unknown counts fall back to the full parse; never assume no shrink.
                existing_text = None
                ...
                if existing_msg_count > incoming_msg_count:
                    ...
                    if existing_text is None:
                        # Only shrink paths need the full snapshot for dominance.
                        existing_text = self.path.read_text(encoding='utf-8')
                    try:
                        existing = json.loads(existing_text)
                    except (json.JSONDecodeError, ValueError):
    ...
    def load(cls, sid):
        ...
        for attempt in range(2):
            ...
            # (When collapsed_partials fired, the fenced save above already
            # returned, so no legacy caching is needed.)
            #

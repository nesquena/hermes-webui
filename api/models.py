...
# Unlike every sibling cache in this module (_SIDECAR_METADATA_CACHE,
# _LEGACY_SIDECAR_FACTS, _CLAUDE_CODE_PARSE_CACHE), this cache_key folds in a
# SQLite content fingerprint that advances on ~every state.db write (see
# _sqlite_file_stat_cache_key / _cli_sessions_streaming_freeze_marker above).
# Each distinct fingerprint mints a NEW dict entry holding a deep copy of the
# full projected session list; without a cap, a long-running process
# accumulates one such entry per unique fingerprint FOREVER (only a full
# clear_cli_sessions_cache() removes anything, and that's only called on
# specific mutation routes, never on a timer). Bounded the same way the other
# caches are, so a long-lived server can't leak unbounded RAM here (memory
# leak investigation, three-week +5MB/hr RSS growth traced to this cache).
_CLI_SESSIONS_CACHE_MAX = 32
...
def _cache_cli_sessions_if_current(
    ...
    with _CLI_SESSIONS_CACHE_LOCK:
        ...
        # Bound the cache the same way its siblings are bounded (#leak-fix):
        # each entry is a full deep-copied session list, so unlike the other
        # OrderedDict caches in this module this one is capped small.
        while len(_CLI_SESSIONS_CACHE) > _CLI_SESSIONS_CACHE_MAX:

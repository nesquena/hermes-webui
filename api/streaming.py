...
def _exact_out_of_band_history_size(result_messages, previous_context):
    ...
    Conclusive only when every later row is assistant/tool output (or a
    compression marker), the durable context contains a real user row, and the
    result prefix equals the complete context under a strict canonical digest:
    either the persisted rows themselves, or their exact Agent-bound projection
    (``_sanitize_messages_for_agent``), which is what the Agent receives and
    replays.  The projection strips WebUI-only metadata (ids, timestamps, turn
    tokens) but keeps every provider-facing field, so visible-text lookalikes
    and payload-distinct rows still fail closed (``None``).
    """
    result_messages = list(result_messages or [])
    previous_context = list(previous_context or [])
    history_size = len(previous_context)
    delta = result_messages[history_size:]
    if not (
        previous_context
        and delta
        and any(
    ...
    if _messages_have_prefix(
        ...
        return history_size
    try:
        agent_bound = _sanitize_messages_for_agent(previous_context)
    except Exception:
        return None
    # A projection that drops or merges rows cannot be aligned by index with
    # the durable context; fail closed instead of guessing the boundary.
    if len(agent_bound) != history_size:
        return None
    for replayed, projected in zip(result_messages[:history_size], agent_bound, strict=True):
        replayed_key = _canonical_replay_digest(replayed)
        if replayed_key is None or replayed_key != _canonical_replay_digest(projected):
            return None
    return history_size
...
def _result_has_authoritative_full_history_prefix(
    ...
    if _exact_out_of_band_history_size(result_messages, previous_context) is not None:
        return True
...
def _prepare_marker_clean_writeback(
    ...
    if cleaned:
        ...
        # An exact replay of the whole durable context followed only by
        # assistant/tool output proves every replayed row is history, even when
        # a repeated prompt leaves the visible-text fallback at boundary 0.
        # Otherwise display restoration would treat the replayed history as
        # current output while model context keeps the answer.  Exact Agent
        # authority placing the current user inside that history still wins.
        exact_history_size = _exact_out_of_band_history_size(
            cleaned, previous_context_messages,
        )
        if (
            exact_history_size is not None
            and boundary < exact_history_size
            and not (
                _active_turn_boundary_is_valid(active_turn_identity)
                and active_turn_identity['current_turn_user_idx']
                < exact_history_size
            )
        ):
            boundary = exact_history_size
        return (
            cleaned,
    ...
    return [], repaired_context, provenance, 0
...
def _display_backfill_identity_details(message):
    ...
    typed_ids = set()
    has_untyped_id = False
    for key in ('id', 'message_id'):
        value = message.get(key) if key in message else None
        if value is None or (type(value) is str and value == ''):
            continue
        typed_id = _strict_incomplete_message_id_key(value)
        if typed_id is None:
            # A bool/container id carries no stable identity, but the row is
            # still comparable: keep the raw id inside the type-faithful common
            # digest below, so only a byte-identical copy (``True`` never
            # equals ``1``) can join it across projections.
            has_untyped_id = True
            continue
        typed_ids.add(typed_id)
    if len(typed_ids) > 1 or (typed_ids and has_untyped_id):
        return None
    stable_id = next(iter(typed_ids)) if typed_ids else None
    excluded_id_keys = set() if has_untyped_id else {'id', 'message_id'}
    ...
    common_digest = _canonical_message_digest(
        {
            ...
            and key not in excluded_id_keys
        }
    )

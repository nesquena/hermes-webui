...
_GENERATED_NO_RESPONSE_ERROR_CONTENT = (
...
def _is_generated_no_response_sidecar(message) -> bool:
    """Match only the exact generic card generated by silent settlement.

    Provider/auth/cancellation/tool-limit cards may also carry ``_error`` and
    must never be retired by the recovered-answer repair.
    """
    return bool(
        isinstance(message, dict)
        and message.get('role') == 'assistant'
        and message.get('_error') is True
        and message.get('content') == _GENERATED_NO_RESPONSE_ERROR_CONTENT
        and message.get('provider_details') == 'No response from provider.'
        and not message.get('provider_details_label')
        and not message.get('_compressionRecovery')
        and not message.get('compression_recovery')
    )


def _is_matching_completed_assistant(message, expected_text: str) -> bool:
    return bool(
        isinstance(message, dict)
        and message.get('role') == 'assistant'
        and not message.get('_error')
        and not message.get('tool_calls')
        and _message_text(message.get('content')) == expected_text
        and (
            message.get('_db_persisted')
            or str(message.get('finish_reason') or '').lower() == 'stop'
        )
    )


def _is_matching_partial_assistant(message, expected_text: str) -> bool:
    return bool(
        isinstance(message, dict)
        and message.get('role') == 'assistant'
        and message.get('_partial') is True
        and not message.get('tool_calls')
        and not message.get('_partial_tool_calls')
        and _message_text(message.get('content')) == expected_text
    )


def _split_stale_no_response_tail(messages, msg_text, recovered_text):
    """Return ``(prefix, pending_user)`` for one proven stale sidecar tail."""
    rows = list(messages or [])
    if len(rows) < 3 or not _is_generated_no_response_sidecar(rows[-1]):
        return None
    if (
        _is_matching_partial_assistant(rows[-2], recovered_text)
        and _looks_like_current_user_turn(rows[-3], msg_text)
    ):
        return rows[:-3], rows[-3]
    if (
        _is_matching_completed_assistant(rows[-3], recovered_text)
        and _looks_like_current_user_turn(rows[-2], msg_text)
    ):
        return rows[:-3], rows[-2]
    if (
        len(rows) >= 4
        and _looks_like_current_user_turn(rows[-4], msg_text)
        and _is_matching_completed_assistant(rows[-3], recovered_text)
        and _is_matching_partial_assistant(rows[-2], recovered_text)
    ):
        return rows[:-4], rows[-4]
    return None


def _retire_recovered_final_answer_sidecar(
    previous_display,
    previous_context,
    result,
    result_messages,
    ...
    *,
    ...
    """Retire a proven stale ``_partial`` + generic no-response tail.

    Recovery can return only the completed assistant row while the WebUI
    sidecar still ends with ``user -> matching _partial -> generated error``.
    Treating that assistant-only delta as a replay emits the same false card
    again.  This helper is deliberately narrow: only a soft partial result
    without an explicit error qualifies, and both the partial text and the
    generated card fingerprint must match exactly.
    """
    display = list(previous_display or [])
    context = list(previous_context or [])
    if not isinstance(result, dict) or tool_limit_reached or last_error:
        return display, context, False

    status = str(result.get('status') or result.get('state') or '').strip().lower()
    if status in {'failed', 'error', 'compression_exhausted'}:
        return display, context, False
    if not (status == 'partial' or bool(result.get('partial'))):
        return display, context, False
    if result.get('failed') or result.get('compression_exhausted'):
        return display, context, False

    significant_result = [
        msg for msg in list(result_messages or [])
        if not _is_context_compression_marker(msg)
        and not _is_compressed_context_tool_result_summary_message(msg)
    ]
    if not significant_result:
        return display, context, False
    recovered = significant_result[-1]
    if (
        not isinstance(recovered, dict)
        or recovered.get('role') != 'assistant'
        or recovered.get('_error')
        or recovered.get('_partial')
        or recovered.get('tool_calls')
    ):
        return display, context, False
    recovered_text = _message_text(recovered.get('content'))
    if not recovered_text:
        return display, context, False

    split_display = _split_stale_no_response_tail(display, msg_text, recovered_text)
    if split_display is None:
        return display, context, False
    prefix, pending_user = split_display

    # state.db reconciliation can insert the same completed answer immediately
    # before the user/partial/error tail. The recovered result below re-adds one
    # final assistant after the user, so drop only that exact durable duplicate.
    if prefix and _is_matching_completed_assistant(prefix[-1], recovered_text):
        prefix = prefix[:-1]
    ...
    # If context_messages was reconstructed from the stale visible sidecar,
    # remove that current-turn fallback too. Otherwise preserve real context.
    split_context = _split_stale_no_response_tail(context, msg_text, recovered_text)
    if split_context is not None:
        context = split_context[0]

    return display, context, True
            ...
            with _agent_lock:
                ...
                with _stream_writeback_stage(_writeback_timings, "merge_result"):
                    ...
                    (
                        _previous_messages,
                        _previous_owner_context_messages,
                        _recovered_stale_no_response_sidecar,
                        ...
                        _result_messages,
                        msg_text,
                        last_error=_recovery_last_error,
                ...
                if (
                    ...
                    and (
                        not _saved_transcript_lacks_final_answer
                        or _recovered_stale_no_response_sidecar
                    )
                ):
                ...
                with _stream_writeback_stage(_writeback_timings, "session_save"):
                    s.save()
                    if _recovered_stale_no_response_sidecar:
                        # The intentional sidecar repair shrinks the visible
                        # transcript. Session.save() correctly snapshots any
                        # shrink to .json.bak, but generic recovery would then
                        # resurrect the exact partial/error rows we just proved
                        ...
                        try:
                            ...
                            logger.warning(
                                "Could not remove stale no-response recovery backup for %s",
                                s.session_id,
                                exc_info=True,
                            )
                if cancel_event.is_set():

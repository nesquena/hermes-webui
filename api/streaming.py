...
# ── Subprocess isolation for agent runs (Issue #2024) ────────────────────────
# Uses multiprocessing.Process to run AIAgent in a separate Python process,
# avoiding GIL contention that stalls the threaded HTTP server.
_AGENT_WORKER_PROCESS: multiprocessing.Process | None = None
_AGENT_WORKER_PROCESS_LOCK = threading.Lock()
_AGENT_WORKER_MP_QUEUES: dict = {}  # stream_id -> Queue
_AGENT_WORKER_CANCEL_EVENTS: dict = {}  # stream_id -> Event
_AGENT_WORKER_RESULTS: dict = {}  # stream_id -> final result dict (set by relay thread)


def _start_agent_subprocess(
    stream_id: str,
    mp_queue: multiprocessing.Queue,
    mp_cancel: multiprocessing.Event,
    **kwargs,
) -> multiprocessing.Process:
    """Spawn a single-use subprocess that runs AIAgent and pushes events to mp_queue."""
    from api.agent_subprocess import run_agent_conversation

    p = multiprocessing.Process(
        target=run_agent_conversation,
        args=(mp_queue, mp_cancel),
        kwargs=kwargs,
        daemon=False,  # daemon processes can't spawn children; use explicit join
    )
    p.start()
    return p


def _terminate_agent_subprocess(stream_id: str) -> None:
    """Signal and kill the subprocess for a given stream."""
    with _AGENT_WORKER_PROCESS_LOCK:
        p = _AGENT_WORKER_PROCESS
...
def _relay_subprocess_events(
    stream_id: str,
    mp_queue: multiprocessing.Queue,
    cancel_event: threading.Event,
    put,
    _live_prompt_estimate_tokens,
    _live_prompt_exact_tokens,
    _live_prompt_estimate_seen_ids,
    _checkpoint_activity,
):
    """Read events from subprocess MP queue and relay to STREAMS via put()."""
    import queue as _queue_module
    while True:
        try:
            item = mp_queue.get(timeout=1.0)
        ...
        if event_type == "done":
            _AGENT_WORKER_RESULTS[stream_id] = data
            put("done", {"session": data, "usage": {}})
            break
        if event_type == "error":
            _AGENT_WORKER_RESULTS[stream_id] = {"_error": True, **data}
            put("apperror", data)
            break
        if event_type == "cancel":
            _AGENT_WORKER_RESULTS[stream_id] = {"cancelled": True}
            put("cancel", data)
            break
        # Pass through: token, reasoning, tool, tool_complete, interim_assistant, status, compressing, warning
        put(event_type, data)


def _run_agent_streaming_subprocess(
    session_id,
    msg_text,
    model,
    workspace,
    ...
    *,
    ...
    """Run AIAgent in a subprocess, relaying SSE events back to STREAMS."""
    from api.agent_subprocess import run_agent_conversation
    from api.models import get_session as _get_session_local
    ...
    s = None
    try:
        s = _get_session_local(session_id)
        ...
        if cancel_event.is_set():
            ...
            _emit_cancel(stream_id, cancel_event, run_journal)
            return

        # Resolve profile home
        try:
        ...
        # Persist user message before subprocess starts
        with _agent_lock:
        ...
        _provider_context_json = _json.dumps({"provider": model_provider} if model_provider else {})
        _attachments_json = _json.dumps(_attachments)
        _hermes_home = os.environ.get("HERMES_HOME", str(Path.home() / ".hermes"))

        # Create MP IPC objects
        mp_queue = multiprocessing.Queue()
        mp_cancel = multiprocessing.Event()
        _AGENT_WORKER_MP_QUEUES[stream_id] = mp_queue
        _AGENT_WORKER_CANCEL_EVENTS[stream_id] = mp_cancel
        ...
        _relay_thread = threading.Thread(
            target=_relay_subprocess_events,
            args=(stream_id, mp_queue, cancel_event, put, [0], [0], set(), [0]),
            daemon=True,
        ...
        proc = _start_agent_subprocess(
            ...
            provider_context_json=_provider_context_json,
            attachments_json=_attachments_json,
            profile_home=_profile_home,
            hermes_home=_hermes_home,
        )
        with _AGENT_WORKER_PROCESS_LOCK:
            _AGENT_WORKER_PROCESS = proc

        # Wait for subprocess completion
        proc.join()

        # Check if process exited cleanly
        if proc.exitcode != 0 and not cancel_event.is_set():
            logger.error("Agent subprocess exited with code %s for stream %s", proc.exitcode, stream_id)
            put("apperror", {"message": f"Agent process exited with code {proc.exitcode}", "type": "subprocess_error"})
            return

        # Wait for relay thread to capture the final result
        _relay_thread.join(timeout=5.0)

        # Retrieve the final result captured by the relay thread
        result = _AGENT_WORKER_RESULTS.pop(stream_id, None)
        
        # If relay thread missed the final event, drain the queue directly
        if result is None:
            while True:
                import queue as _queue_module
                try:
                    item = mp_queue.get(timeout=0.5)
                except _queue_module.Empty:
                    break
                if item is None:
                    break
        ...
        # If we still don't have a result, report an error
        if result is None:
            put("apperror", {"message": "No result from agent subprocess", "type": "subprocess_error"})
            return
        
        # Handle cancel during run
        if cancel_event.is_set() or (isinstance(result, dict) and result.get("cancelled")):
            if not ephemeral:
                with _agent_lock:
        ...
        if isinstance(result, dict) and "messages" in result:
            _result_messages = result["messages"]
            input_tokens = result.get("input_tokens", 0)
            output_tokens = result.get("output_tokens", 0)
            estimated_cost = result.get("estimated_cost")
            cache_read_tokens = result.get("cache_read_tokens", 0)
            cache_write_tokens = result.get("cache_write_tokens", 0)
            _compressed = result.get("compressed", False)

            # Merge messages into session
            ...
            with _agent_lock:
                ...
                if result.get("context_length"):
                    s.context_length = result["context_length"]
                if result.get("threshold_tokens"):
                    s.threshold_tokens = result["threshold_tokens"]
                if result.get("last_prompt_tokens"):
                    s.last_prompt_tokens = result["last_prompt_tokens"]
            ...
            if _compressed:
                put("compressed", {
                    ...
                    "continuation_session_id": result.get("session_id", session_id),
            ...
            if result.get("context_length"):
                usage["context_length"] = result["context_length"]
                usage["threshold_tokens"] = result.get("threshold_tokens", 0)
                usage["last_prompt_tokens"] = result.get("last_prompt_tokens", 0)

            raw_session = s.compact() | {"messages": s.messages}
            put("done", {"session": redact_session_data(raw_session), "usage": usage})
            put("stream_end", {"session_id": session_id})
        else:
            _err = result.get("error", "Unknown error from agent subprocess") if isinstance(result, dict) else "Unknown error"
            put("apperror", {"message": _err, "type": "agent_error"})

    except Exception as e:
        logger.error("Agent streaming subprocess error: %s", e, exc_info=True)
        try:
            q = STREAMS.get(stream_id)
            if q:
        ...
        _AGENT_WORKER_MP_QUEUES.pop(stream_id, None)
        _AGENT_WORKER_CANCEL_EVENTS.pop(stream_id, None)
        _AGENT_WORKER_RESULTS.pop(stream_id, None)
        with _AGENT_WORKER_PROCESS_LOCK:
            if _AGENT_WORKER_PROCESS is not None and not _AGENT_WORKER_PROCESS.is_alive():
                _AGENT_WORKER_PROCESS = None


def _emit_cancel(stream_id, cancel_event, run_journal):
    """Emit cancel event to the queue."""
    q = STREAMS.get(stream_id)
    if q is None:
        return
    try:
...
def cancel_stream(stream_id: str) -> bool:
    ...
    with streams_lock:
        ...
        # Signal the subprocess cancel event if this stream is managed by the worker pool
        try:
            from api.streaming import _AGENT_WORKER_CANCEL_EVENTS
            mp_cancel = _AGENT_WORKER_CANCEL_EVENTS.get(stream_id)
            if mp_cancel is not None:
                mp_cancel.set()
        except Exception:
            pass

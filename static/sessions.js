...
// #7549: with the all-profiles sidebar on, a row may belong to a profile other
// than the active one. The archive endpoint resolves the session against the
// profile named in the request, so every archive request has to carry the row's
// own profile — otherwise the server looks the row up under the active profile
// and answers 404. Mirrors _externalImportPayload() above.
function _archivePayload(session, archived=true, sessionId=null) {
  const payload = {session_id: sessionId || (session && session.session_id), archived};
  if (_showAllProfiles && session && typeof session.profile === 'string' && session.profile) {
    payload.all_profiles = true;
    payload.profile = session.profile;
  }
  ...
  archiveBtn.onclick=async()=>{
    ...
    try{
      const results=await Promise.all(ids.map(async sid=>{
        const response=await api('/api/session/archive',{method:'POST',body:JSON.stringify(_archivePayload(sessionsById.get(sid)||null,true,sid))});
...
async function _archiveSession(session, archived=true, beforeListRender=null){
  ...
  try{
    const response=await api('/api/session/archive',{method:'POST',body:JSON.stringify(_archivePayload(session,archived))});
  ...
  if(isExternalSession && !session.archived){
    menu.appendChild(_buildSessionAction(
      ...
      async()=>{
        closeSessionActionMenu();
        try{
          await api('/api/session/archive',{method:'POST',body:JSON.stringify(_archivePayload(session,true))});

...
async function newSession(flash, options={}){
  ...
  _newSessionInFlight=(async()=>{
    ...
    if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(data.session); S.messages=data.session.messages||[];
  ...
  deleteBtn.onclick=async()=>{
    ...
    try{
      ...
      if(S.session&&ids.includes(S.session.session_id)){
        if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(null);S.messages=[];S.entries=[];localStorage.removeItem('hermes-webui-session');
...
async function deleteSession(sid, beforeDelete=null){
  ...
  if(S.session&&S.session.session_id===sid){
    if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(null);S.messages=[];S.entries=[];
    if(typeof _hydrateTodosFromSession==='function') _hydrateTodosFromSession(null);
    ...
    if(remaining.sessions&&remaining.sessions.length){
      ...
      $('fileTree').innerHTML='';
      if(typeof S!=='undefined' && typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(null);

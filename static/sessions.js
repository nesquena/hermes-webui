...
async function loadSession(sid){
  ...
  if (currentSid !== sid || forceReload) {
    ...
    // #5177 keep-stale-until-loaded defers destructive clearing on same-session
    // refreshes; display-cache restores also keep the already-restored DOM/state.
    if(!_restoredSessionDisplayBeforeFetch && !_keepStaleUntilLoaded){
  ...
  if(INFLIGHT[sid]){
    ...
    if(typeof clearLiveToolCards==='function') clearLiveToolCards();
    try {
      await _ensureMessagesLoaded(sid,{force:_sameSessionServerAhead||_keepStaleUntilLoaded});
    } catch(e) {
...
async function _ensureMessagesLoaded(sid, opts={}) {
  opts = opts || {};
  const forceReloadMessages = !!opts.force;
  // Already have messages? (e.g. from INFLIGHT restore path, already set).
  // A same-session metadata refresh or keep-stale-until-loaded path passes force
  // so the old tab fetches and swaps the authoritative transcript.

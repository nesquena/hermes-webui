...
const LOCALES = {
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    profiles_vs_workspaces_title: 'Profiles vs workspaces',
    profiles_vs_workspaces_subtitle: 'Use profiles for how the agent works; use workspaces for what files it works on.',
    profiles_vs_workspaces_heading: 'Use profiles for how; workspaces for what',
    profiles_vs_workspaces_profiles_label: 'Profiles',
    profiles_vs_workspaces_profiles_body: 'Agent identity, memory, skills, model/provider config, and connected tools. Create profiles for roles like researcher, writer, marketer, or developer when those roles should carry different context or capabilities.',
    profiles_vs_workspaces_workspaces_label: 'Workspaces',
    profiles_vs_workspaces_workspaces_body: 'Project or product folders on disk. Use one workspace per repo/product so chat, terminal, and file browsing point at the right files.',
    profiles_vs_workspaces_together_label: 'Together',
    profiles_vs_workspaces_together_body: 'A profile can have a default workspace, but you can still switch workspaces for a session. Profiles answer “who is working?”; workspaces answer “where are they working?”',
  ...
  folder_add_as_space_title: 'Aggiungere come Spazio?',
    ...
    profiles_vs_workspaces_title: 'Profili e workspace',
    profiles_vs_workspaces_subtitle: 'Usa i profili per come lavora l’agente; usa i workspace per i file su cui lavora.',
    profiles_vs_workspaces_heading: 'Usa i profili per il come; i workspace per il cosa',
    profiles_vs_workspaces_profiles_label: 'Profili',
    profiles_vs_workspaces_profiles_body: 'Identità dell’agente, memoria, skill, configurazione modello/provider e strumenti collegati. Crea profili per ruoli come ricercatore, scrittore, marketer o sviluppatore quando quei ruoli devono avere contesto o capacità diverse.',
    profiles_vs_workspaces_workspaces_label: 'Workspace',
    profiles_vs_workspaces_workspaces_body: 'Cartelle di progetto o prodotto su disco. Usa un workspace per repo/prodotto così chat, terminale e navigazione file puntano ai file giusti.',
    profiles_vs_workspaces_together_label: 'Insieme',
    profiles_vs_workspaces_together_body: 'Un profilo può avere un workspace predefinito, ma puoi comunque cambiare workspace per una sessione. I profili rispondono a “chi sta lavorando?”; i workspace rispondono a “dove sta lavorando?”',
  ...
  folder_add_as_space_title: 'スペースとして追加?',
    ...
    profiles_vs_workspaces_title: 'プロファイルとワークスペース',
    profiles_vs_workspaces_subtitle: 'プロファイルはエージェントの働き方、ワークスペースは扱うファイルを決めます。',
    profiles_vs_workspaces_heading: 'プロファイルは「どう働くか」、ワークスペースは「何を扱うか」',
    profiles_vs_workspaces_profiles_label: 'プロファイル',
    profiles_vs_workspaces_profiles_body: 'エージェントの個性、メモリ、スキル、モデル/プロバイダー設定、接続ツールです。研究者、ライター、マーケター、開発者など、異なる文脈や機能を持たせたい役割ごとにプロファイルを作成します。',
    profiles_vs_workspaces_workspaces_label: 'ワークスペース',
    profiles_vs_workspaces_workspaces_body: 'ディスク上のプロジェクトまたはプロダクトフォルダです。リポジトリ/プロダクトごとに1つ使うと、チャット、ターミナル、ファイル閲覧が正しいファイルを指します。',
    profiles_vs_workspaces_together_label: '組み合わせ',
    profiles_vs_workspaces_together_body: 'プロファイルには既定のワークスペースを設定できますが、セッションごとにワークスペースを切り替えることもできます。プロファイルは「誰が働くか」、ワークスペースは「どこで働くか」に答えます。',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    profiles_vs_workspaces_title: 'Профили и рабочие области',
    profiles_vs_workspaces_subtitle: 'Профили задают, как работает агент; рабочие области — с какими файлами он работает.',
    profiles_vs_workspaces_heading: 'Профили — про как; рабочие области — про что',
    profiles_vs_workspaces_profiles_label: 'Профили',
    profiles_vs_workspaces_profiles_body: 'Идентичность агента, память, навыки, настройки модели/провайдера и подключенные инструменты. Создавайте профили для ролей вроде исследователя, автора, маркетолога или разработчика, когда им нужны разные контексты или возможности.',
    profiles_vs_workspaces_workspaces_label: 'Рабочие области',
    profiles_vs_workspaces_workspaces_body: 'Папки проекта или продукта на диске. Используйте одну рабочую область на репозиторий/продукт, чтобы чат, терминал и просмотр файлов указывали на правильные файлы.',
    profiles_vs_workspaces_together_label: 'Вместе',
    profiles_vs_workspaces_together_body: 'У профиля может быть рабочая область по умолчанию, но для сеанса ее все равно можно сменить. Профили отвечают на вопрос «кто работает?»; рабочие области — «где он работает?»',
  ...
  folder_add_as_space_title: '¿Añadir como espacio?',
    ...
    workspace_switched_to: (name) => `Switched to ${name}`,
    profiles_no_profiles: 'No profiles found.',
    profiles_vs_workspaces_title: 'Perfiles y espacios de trabajo',
    profiles_vs_workspaces_subtitle: 'Usa perfiles para cómo trabaja el agente; usa espacios de trabajo para los archivos sobre los que trabaja.',
    profiles_vs_workspaces_heading: 'Perfiles para el cómo; espacios de trabajo para el qué',
    profiles_vs_workspaces_profiles_label: 'Perfiles',
    profiles_vs_workspaces_profiles_body: 'Identidad del agente, memoria, habilidades, configuración de modelo/proveedor y herramientas conectadas. Crea perfiles para roles como investigador, redactor, marketer o desarrollador cuando esos roles deban llevar contexto o capacidades distintas.',
    profiles_vs_workspaces_workspaces_label: 'Espacios de trabajo',
    profiles_vs_workspaces_workspaces_body: 'Carpetas de proyecto o producto en disco. Usa un espacio de trabajo por repositorio/producto para que el chat, la terminal y el explorador de archivos apunten a los archivos correctos.',
    profiles_vs_workspaces_together_label: 'Juntos',
    profiles_vs_workspaces_together_body: 'Un perfil puede tener un espacio de trabajo predeterminado, pero aún puedes cambiar de espacio de trabajo para una sesión. Los perfiles responden “¿quién trabaja?”; los espacios de trabajo responden “¿dónde trabaja?”',
    profile_api_keys_configured: 'API keys configured',
    profile_gateway_running: 'Gateway running',
    profile_gateway_stopped: 'Gateway stopped',
    profile_active: 'ACTIVE',
  ...
  folder_add_as_space_title: 'Als Bereich hinzufügen?',
    ...
    profiles_vs_workspaces_title: 'Profile und Workspaces',
    profiles_vs_workspaces_subtitle: 'Profile steuern, wie der Agent arbeitet; Workspaces steuern, mit welchen Dateien er arbeitet.',
    profiles_vs_workspaces_heading: 'Profile für das Wie; Workspaces für das Was',
    profiles_vs_workspaces_profiles_label: 'Profile',
    profiles_vs_workspaces_profiles_body: 'Agent-Identität, Memory, Skills, Modell-/Provider-Konfiguration und verbundene Tools. Erstelle Profile für Rollen wie Researcher, Writer, Marketer oder Developer, wenn diese Rollen unterschiedlichen Kontext oder Fähigkeiten brauchen.',
    profiles_vs_workspaces_workspaces_label: 'Workspaces',
    profiles_vs_workspaces_workspaces_body: 'Projekt- oder Produktordner auf der Festplatte. Nutze einen Workspace pro Repo/Produkt, damit Chat, Terminal und Dateiansicht auf die richtigen Dateien zeigen.',
    profiles_vs_workspaces_together_label: 'Zusammen',
    profiles_vs_workspaces_together_body: 'Ein Profil kann einen Standard-Workspace haben, aber du kannst den Workspace für eine Sitzung trotzdem wechseln. Profile beantworten „wer arbeitet?“; Workspaces beantworten „wo arbeitet er?“',
  ...
  folder_add_as_space_title: '添加为工作区？',
    ...
    profiles_vs_workspaces_title: '配置档与工作区',
    profiles_vs_workspaces_subtitle: '配置档决定智能体如何工作；工作区决定它处理哪些文件。',
    profiles_vs_workspaces_heading: '配置档管“如何”；工作区管“什么”',
    profiles_vs_workspaces_profiles_label: '配置档',
    profiles_vs_workspaces_profiles_body: '智能体身份、记忆、技能、模型/提供商配置以及已连接的工具。需要不同上下文或能力时，可为研究员、写作者、营销人员或开发者等角色创建配置档。',
    profiles_vs_workspaces_workspaces_label: '工作区',
    profiles_vs_workspaces_workspaces_body: '磁盘上的项目或产品文件夹。每个仓库/产品使用一个工作区，让聊天、终端和文件浏览都指向正确文件。',
    profiles_vs_workspaces_together_label: '配合使用',
    profiles_vs_workspaces_together_body: '配置档可以有默认工作区，但你仍可为单个会话切换工作区。配置档回答“谁在工作？”；工作区回答“在哪里工作？”',
  ...
  folder_add_as_space_title: '\u65b0\u589e\u70ba\u5de5\u4f5c\u5340\uff1f',
    ...
    profiles_vs_workspaces_title: '設定檔與工作區',
    profiles_vs_workspaces_subtitle: '設定檔決定智慧體如何工作；工作區決定它處理哪些檔案。',
    profiles_vs_workspaces_heading: '設定檔管「如何」；工作區管「什麼」',
    profiles_vs_workspaces_profiles_label: '設定檔',
    profiles_vs_workspaces_profiles_body: '智慧體身分、記憶、技能、模型/提供者設定，以及已連接的工具。當研究員、寫作者、行銷人員或開發者等角色需要不同脈絡或能力時，請為它們建立設定檔。',
    profiles_vs_workspaces_workspaces_label: '工作區',
    profiles_vs_workspaces_workspaces_body: '磁碟上的專案或產品資料夾。每個 repo/產品使用一個工作區，讓聊天、終端機與檔案瀏覽都指向正確檔案。',
    profiles_vs_workspaces_together_label: '搭配使用',
    profiles_vs_workspaces_together_body: '設定檔可以有預設工作區，但你仍可為單一工作階段切換工作區。設定檔回答「誰在工作？」；工作區回答「在哪裡工作？」',
    remove: '移除',
    remove_failed: '\u79fb\u9664\u5931\u6557\uff1a',
  ...
  ko: {
    ...
    workspace_switched_to: (name) => `Switched to ${name}`,
    profiles_no_profiles: 'No profiles found.',
    profiles_vs_workspaces_title: '프로필과 워크스페이스',
    profiles_vs_workspaces_subtitle: '프로필은 에이전트가 어떻게 일하는지, 워크스페이스는 어떤 파일을 다루는지 정합니다.',
    profiles_vs_workspaces_heading: '프로필은 어떻게, 워크스페이스는 무엇을',
    profiles_vs_workspaces_profiles_label: '프로필',
    profiles_vs_workspaces_profiles_body: '에이전트 정체성, 메모리, 스킬, 모델/제공자 설정, 연결된 도구입니다. 연구자, 작성자, 마케터, 개발자처럼 서로 다른 맥락이나 기능이 필요한 역할별로 프로필을 만드세요.',
    profiles_vs_workspaces_workspaces_label: '워크스페이스',
    profiles_vs_workspaces_workspaces_body: '디스크의 프로젝트 또는 제품 폴더입니다. 저장소/제품마다 하나의 워크스페이스를 사용하면 채팅, 터미널, 파일 탐색이 올바른 파일을 가리킵니다.',
    profiles_vs_workspaces_together_label: '함께 사용',
    profiles_vs_workspaces_together_body: '프로필에는 기본 워크스페이스가 있을 수 있지만, 세션별로 워크스페이스를 바꿀 수도 있습니다. 프로필은 “누가 일하는가?”에 답하고, 워크스페이스는 “어디에서 일하는가?”에 답합니다.',
    profile_api_keys_configured: 'API keys configured',
    profile_gateway_running: 'Gateway running',
    profile_gateway_stopped: 'Gateway stopped',
    profile_active: 'ACTIVE',
  ...
  fr: {
    ...
    profiles_vs_workspaces_title: 'Profils et espaces de travail',
    profiles_vs_workspaces_subtitle: 'Utilisez les profils pour la façon dont l’agent travaille ; les espaces de travail pour les fichiers sur lesquels il travaille.',
    profiles_vs_workspaces_heading: 'Profils pour le comment ; espaces de travail pour le quoi',
    profiles_vs_workspaces_profiles_label: 'Profils',
    profiles_vs_workspaces_profiles_body: 'Identité de l’agent, mémoire, compétences, configuration modèle/fournisseur et outils connectés. Créez des profils pour des rôles comme chercheur, rédacteur, marketeur ou développeur quand ces rôles doivent porter un contexte ou des capacités différents.',
    profiles_vs_workspaces_workspaces_label: 'Espaces de travail',
    profiles_vs_workspaces_workspaces_body: 'Dossiers de projet ou de produit sur le disque. Utilisez un espace de travail par dépôt/produit afin que le chat, le terminal et l’exploration de fichiers pointent vers les bons fichiers.',
    profiles_vs_workspaces_together_label: 'Ensemble',
    profiles_vs_workspaces_together_body: 'Un profil peut avoir un espace de travail par défaut, mais vous pouvez toujours changer d’espace de travail pour une session. Les profils répondent à « qui travaille ? » ; les espaces de travail répondent à « où travaille-t-il ? »',

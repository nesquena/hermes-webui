...
function _hideSettingsPanel(){
  _resetSettingsPanelState();
  // Every teardown path must cancel the ops-status poll chain, not just the
  // section switch: leaving the System section by CLOSING the panel used to
  // keep a 2s poll running against a card that is no longer on screen.
  if(typeof _stopOpsActionsPolling==='function') _stopOpsActionsPolling();

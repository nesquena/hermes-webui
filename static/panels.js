...
function _profileDropdownCacheUsable(data){
  return !!(data && Array.isArray(data.profiles) && data.profiles.length);
}
...
function _profileDropdownReadStoredCache(){
  ...
  try{
    ...
    if(Date.now()-parsed.ts>PROFILE_DROPDOWN_CACHE_TTL_MS) return null;
    if(!_profileDropdownCacheUsable(parsed.data)) return null;
...
function _profileDropdownWriteStoredCache(data){
  if(!_profileDropdownCacheUsable(data)) return;

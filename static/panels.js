...
async function _saveUpdateChannelFromSelector(channelSel){
  ...
  if(typeof _setPreferencesAutosaveStatus==='function') _setPreferencesAutosaveStatus('saving');
  try{
    ...
    // Re-sync the selector from the server response. Defensive fallback to
    // 'stable' guards against a malformed response with no update_channel key.
    const confirmed=(saved&&(saved.update_channel==='experimental'||saved.update_channel==='stable'))
      ?saved.update_channel:'stable';
    _confirmedUpdateChannel=confirmed;
    channelSel.value=confirmed;
    if(typeof _setPreferencesAutosaveStatus==='function') _setPreferencesAutosaveStatus('saved');
    ...
    // controls agree with what the server actually holds. Do not call
    // _setPreferencesAutosaveStatus('failed'): its retry button replays the
    // generic preferences payload, which by design no longer contains
    // update_channel (#6612). Clear the saving indicator instead.
    ...
    if(typeof _setPreferencesAutosaveStatus==='function') _setPreferencesAutosaveStatus(null);
  }
}

...
// null = not yet known. Starting at `true` meant the write controls were live
// before any answer arrived, and a FAILED load never cleared it — so a rejected
// or errored capability probe left the panel fully writable, which is the one
// state it must not default to.
let _mcpWritable=null;
function _mcpDenyWriteCapability(reason){
  _mcpApplyWriteCapability({writable:false, unavailable_reason:reason||''});
}
function _mcpApplyWriteCapability(r){
  ...
  // Explicit true only. `r.writable!==false` treated a missing field, and any
  // shape that is not literally false, as permission to write.
  _mcpWritable=!!(r&&r.writable===true);
  const addBtn=$('mcpAddServerBtn');
  if(addBtn) addBtn.disabled=!_mcpWritable;
  // An open form belongs to the capability that was in force when it opened.
  if(!_mcpWritable) _mcpCloseFormsOnCapabilityLoss();
...
function _mcpCloseFormsOnCapabilityLoss(){
  // Create as well as edit: an already-open create form stayed submittable
  // after write capability went away.
  if(_mcpFormState&&(_mcpFormState.mode==='edit'||_mcpFormState.mode==='create')){
    cancelMcpServerForm();
  }
}
function loadMcpServers(){
  ...
  api('/api/mcp/servers').then(r=>{
    if(!r||!Array.isArray(r.servers)){
      // A rejected or malformed load says nothing about write capability, so
      // it must not leave the previous answer standing.
      _mcpDenyWriteCapability(r&&r.error?String(r.error):'');
      return;
    }
    ...
    // Fail closed on a network/parse failure too: the panel showed an error but
    // kept whatever write capability was last granted.
    _mcpDenyWriteCapability('');
...
function openMcpServerEdit(name){
  ...
  _mcpFormState={mode:'edit',name:server.name,revision:server.revision||null};
...
async function saveMcpServerForm(){
  ...
  // Guard at DISPATCH, not only when the form was drawn. Capability can have
  // gone away since — a failed reload, a profile switch — and a form that is
  // still on screen would otherwise still submit.
  if(_mcpWritable!==true){
    if(errEl){errEl.textContent=t('mcp_write_unavailable');errEl.style.display='';}
    return;
  }
  ...
  const body={};
  // Send the revision this form was populated from, so a concurrent CLI or
  // second-tab edit is refused with 409 instead of being overwritten.
  if(isEdit&&_mcpFormState&&_mcpFormState.revision) body.revision=_mcpFormState.revision;
  if(transport==='url'){

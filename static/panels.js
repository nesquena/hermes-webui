...
function _appendCronPausedSection(box, pausedJobs) {
  ...
  header.onclick = (event) => {
    ...
    _setCronPausedSectionCollapsed(nowCollapsed);
    header.setAttribute('aria-expanded', nowCollapsed ? 'false' : 'true');
    const nextToggleLabel = nowCollapsed ? (t('cron_expand_section') || 'Expand section') : (t('cron_collapse_section') || 'Collapse section');
    header.title = nextToggleLabel;
    header.setAttribute('aria-label', `${nextToggleLabel}: ${sectionTitle}`);
    const chevron = header.querySelector('.cron-group-chevron');

...
(async()=>{
  ...
  if(_testUpdates||(_bootSettings.check_for_updates!==false&&!sessionStorage.getItem('hermes-update-checked')&&!sessionStorage.getItem('hermes-update-dismissed'))){
    ...
    api(_checkUrl,{method:_testUpdates?'GET':'POST',body:_testUpdates?undefined:JSON.stringify({force:false})}).then(d=>{if(!_testUpdates)sessionStorage.setItem('hermes-update-checked','1');const {hasDirty:_dirty}=_updateDirtyState(d);if((d.webui&&d.webui.behind>0)||(d.agent&&d.agent.behind>0)||_dirty)_showUpdateBanner(d);}).catch(()=>{});
  }

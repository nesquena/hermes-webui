...
function renderModelDropdown(){
  ...
  const _filterModels=(term)=>{
    ...
    for(const m of _modelData){
      ...
      // The chip on each row shows the ROUTING provider (where the request
      // actually goes), which is the optgroup label from the server. Users
      // who configure `custom_providers: { crof: {...}, venice: {...} }`
      // see "Crof" / "Venice" instead of "Custom"; the generic "Custom"
      // fall-through is the correct label when no friendly name is set.

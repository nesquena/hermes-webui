...
function _artifactErrorPayload(err){
  ...
  document.addEventListener('click',async ev=>{
    ...
    if(btn){
      ...
      try{
        ...
        // Two steps on purpose: /prepare is the only place a path becomes
        // publish authority. It validates against the requesting profile's
        // roots and returns a short-lived capability bound to this owner,
        // profile and session; /publish refuses without one.
        const prep=await api('/api/artifact/prepare',{
          method:'POST',
          body:JSON.stringify({path,session_id:sid||undefined}),
        });
        if(!prep||!prep.ok||!prep.source) throw new Error((prep&&prep.error)||'publish failed');
        // api() returns the decoded JSON body and throws on non-ok statuses.
        const d=await api('/api/artifact/publish',{
          method:'POST',
          body:JSON.stringify({
            path:prep.source.path,title:title||undefined,session_id:sid||undefined,
            capability:prep.source.capability,
          }),
        });
    ...
    if(pub){
      ...
      try{
        const prep=await api('/api/artifact/prepare',{
          method:'POST',
          body:JSON.stringify({path:pub.dataset.path||''}),
        });
        if(!prep||!prep.ok||!prep.source) throw new Error((prep&&prep.error)||'publish failed');
        const send=async verbatim=>api('/api/artifact/publish',{
          method:'POST',
          body:JSON.stringify({
            path:prep.source.path,token:pub.dataset.token||'',public:true,
            capability:prep.source.capability,
            ...(verbatim?{verbatim_public:true}:{}),
          }),
        });
        let d;
        try{
          ...
          // 409 + needs_verbatim_ack: the format cannot be redacted, so making
          // it public would expose it byte-for-byte. Ask before doing that
          // instead of silently publishing unredacted content.
          const payload=_artifactErrorPayload(err);
          if(!(err&&err.status===409&&payload&&payload.needs_verbatim_ack)) throw err;
          const ok=await showConfirmDialog({
            title:t('artifact_make_public'),
            message:payload.error||t('artifact_verbatim_warning'),
            danger:true,
          });
          if(!ok){pub.disabled=false;return;}
          d=await send(true);
        }
        if(!d||!d.ok) throw new Error((d&&d.error)||'publish failed');
        pub.remove();

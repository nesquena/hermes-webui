...
(function(){
  ...
  function renderDemoSmokeSuiteResult(data){
    ...
    const rows = results.slice(0, 20).map(function(item){
      ...
      const timeTravelSummary = demo === 'demo_time_travel_restore' && timeTravelCheck
        ...
        : '';
      const safeAdminCheck = item && item.safe_admin_recovery_check && typeof item.safe_admin_recovery_check === 'object' && !Array.isArray(item.safe_admin_recovery_check) ? item.safe_admin_recovery_check : null;
      const safeAdminSummary = demo === 'demo_safe_admin_recovery' && safeAdminCheck
        ? '<div class="capy-spaces-muted"><strong>Safe admin recovery checklist</strong></div><div class="capy-spaces-muted">Admin recovery flow: '+(safeAdminCheck.metadata_only === true ? 'metadata-only' : 'metadata pending')+' · '+(safeAdminCheck.generated_widgets_rendered === false ? 'generated widgets not rendered' : 'generated widgets blocked')+' · disabled widgets '+Number(safeAdminCheck.disabled_widget_count || 0)+' · rollback controls '+(safeAdminCheck.rollback_controls_available === true ? 'available' : 'not ready')+' · repair controls '+(safeAdminCheck.repair_controls_available === true ? 'available' : 'not ready')+' · module quarantine '+(safeAdminCheck.module_quarantine_available === true ? 'available' : 'not ready')+'</div>'
        : '';
      return '<div class="capy-spaces-widget"><div><strong>'+escapeHtml(demo)+'</strong>' +
        '<div class="capy-spaces-muted">template: '+escapeHtml(template)+' · widgets: '+widgetCount+' · persisted: '+persistedWidgetCount+' · persistence: '+escapeHtml(persistence)+' · rollback point: '+escapeHtml(rollbackPoint)+'</div>' + flowSummary + weatherObservationSummary + notesSummary + kanbanSummary + researchSummary + timeTravelSummary + safeAdminSummary + '</div></div>';
    }).join('');
  ...
  function renderSpaceUpdateReceipt(data){
    ...
    const preflight = renderPromptPreflightEvidence(result.prompt_preflight);
    const policy = renderActionPolicyEvidence(result.autonomy_policy);
    const progress = renderPackageProgressEvidence(result.progress_event, 'Space update progress');
    const advisory = renderMemoryAdvisoryEvidence(result.memory_advisory);
    const compaction = renderCompactionEvidence(result.output_compaction || result.compaction);
    if (!preflight && !policy && !progress && !advisory && !compaction) return '';
    return '<div class="capy-spaces-card" role="status"><h3>Space update receipt</h3>' +
      '<div class="capy-spaces-muted">Confirmed Space metadata update completed with metadata-only policy, progress, memory advisory/no-authority, and compaction evidence. Raw prompts, widget bodies, memory context, implementation fields, and secrets stay omitted.</div>' +
      preflight + policy + progress + advisory + compaction + '</div>';
  }

  function prependSpaceUpdateReceipt(data){
    const root = document.getElementById('capySpacesRoot');
    if (!root) return;
  ...
  function prependSystemWidgetUpsertReceipt(data){
    const root = document.getElementById('capySpacesRoot');
    if (!root) return;
    const receipt = renderSystemWidgetUpsertReceipt(data);
    if (receipt) root.innerHTML = receipt + root.innerHTML;
  }
  ...
  function isCurrentRevision(rev){
    ...
    return rev.is_current_revision === true || rev.timeline_state === 'current';
  }
  ...
  function isBlockedRuntimeMessageType(type){
    ...
    if (text === 'capy:ready' || text === 'capy:resize' || text === 'capy:agent:prompt') return false;
    return /^capy:/.test(text);
  }
  ...
  async function handleCapySpacesClick(event){
    ...
    if (action === 'addSystemWidget') {
      ...
      if (['chat', 'workspaces', 'tasks', 'memory', 'settings'].indexOf(panel) === -1) return;
      if (!spaceId) return;
      const data = await postSpacesJson('api/spaces/system-widget/upsert', {space_id: spaceId, panel: panel, layout: {x: 0, y: 0, w: 12, h: 6}});
      await loadSpaceWidgets(spaceId);
      prependSystemWidgetUpsertReceipt(data);
      return;
    }
  ...
  function renderRecoveryRevisionRows(spaceId, revisions){
    const actionSpaceId = safePathIdText(spaceId);
  ...
  function renderRecoverySnapshot(data){
    ...
    const rows = spaces.length ? spaces.map(function(s){
      ...
      const spaceAction = !spaceId ? '' : (spaceDisabled
      ...
      const spaceRepairAction = spaceId ? '<button type="button" class="capy-spaces-btn" data-capy-action="repairRecoverySpace" data-space-id="'+escapeHtml(spaceId)+'">Ask Capy to repair Space</button>' : '';
      const spaceExportActions = spaceId ? '<button type="button" class="capy-spaces-btn" data-capy-action="exportRecoverySpaceYaml" data-space-id="'+escapeHtml(spaceId)+'">Export YAML</button>' +
      ...
      const widgetRows = widgets.length ? '<div class="capy-spaces-widget-list">'+widgets.map(function(w){
        ...
        const repairAction = spaceId && actionWidgetId ? '<button type="button" class="capy-spaces-btn" data-capy-action="repairRecoveryWidget" data-space-id="'+escapeHtml(spaceId)+'" data-widget-id="'+escapeHtml(actionWidgetId)+'" data-widget-title="'+escapeHtml(title)+'">Ask Capy to repair</button>' : '';
        const toggleAction = spaceId && actionWidgetId ? (disabled ? '<button type="button" class="capy-spaces-btn" data-capy-action="enableRecoveryWidget" data-space-id="'+escapeHtml(spaceId)+'" data-widget-id="'+escapeHtml(actionWidgetId)+'">Enable widget</button>' : '<button type="button" class="capy-spaces-btn capy-spaces-danger" data-capy-action="disableRecoveryWidget" data-space-id="'+escapeHtml(spaceId)+'" data-widget-id="'+escapeHtml(actionWidgetId)+'">Disable widget</button>') : '';

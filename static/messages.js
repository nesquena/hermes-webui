...
async function send(){
  ...
  try{
    ...
    if(e&&e.status===404){
      ...
      removeThinking();
      if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(null);S.messages=[];
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('done',e=>{
      ...
      const _finishDone=()=>{
        ...
        if(isActiveSession){
          ...
          if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(d.session);S.messages=_carryForwardEphemeralTurnFields(S.messages||[], d.session.messages||[]);if(typeof _messagesTruncated!=='undefined')_messagesTruncated=!!d.session._messages_truncated;
      ...
      if(S.session&&eventMatchesCurrent){
        ...
        try{
          ...
          if(isRecoveryControlMessage){
            ...
            if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(d.session);
    ...
    source.addEventListener('cancel',e=>{
      ...
      const _applyCancelSessionPayload=(sessionPayload)=>{
        ...
        if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(sessionPayload);
  ...
  async function _restoreSettledSession(source, options=null){
    ...
    try{
      ...
      if(isActiveSession){
        S.activeStreamId=null;
        clearLiveToolCards();if(!assistantText)removeThinking();
        if(typeof setWorkspaceSearchSession==='function') setWorkspaceSearchSession(session);

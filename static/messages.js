  ...
  if(S.busy||compressionRunning){
    if(text){
      ...
      if(busyMode==='steer'&&S.activeStreamId&&typeof _trySteer==='function'){
        ...
        // _trySteer falls back to queue-only mode if the agent
        // isn't running / cached / steer-capable, preserving the active stream.
        $('msg').value='';autoResize();
        // Do NOT clear pendingFiles yet — _trySteer may fall back to
        // queue-only and needs the files for queueSessionMessage.

  ...
  function _sourceEventTypeForSnapshotAnchorRow(row){
    ...
    if(role==='lifecycle'||kind==='lifecycle_status'){
      ...
      if(
        ...
        // authority for "compression actually started". Any cue accepted here
        // but not there lets replay invent a "Compressing context" divider the
        // live SSE path never painted — the defect this function prevents.
        //
        // Deliberately NOT cues, because Python rejects them:
        //   - bare `phase==='compressing'`: a genuine compression row arrives
        //     with a canonical source_event_type and returns at the top of this
        //     function, before any text inference. Reaching this branch means
        //     the row carried no canonical type, so phase alone is not
        //     evidence. routes.py also appends a placeholder "live anchor
        //     shell" row (role lifecycle, status running, source_event_type
        //     runtime_journal_snapshot) whenever a stream has events but no
        //     visible rows yet; a bare running/compressing phase classified
        //     every such shell as a compression start, which is the permanent
        //     phantom divider on sessions that never compressed anything.
        //   - `compressing context`: Python keys on `compacting context`;
        //     there is no `compressing context` cue to mirror.
        //   - `preflight compression`: excluded on both sides. It announces
        //     intent, and the later `Compacting context` marker is what proves
        //     compaction proceeded.

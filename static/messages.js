...
async function send(){
  ...
  if(S.busy||compressionRunning){
    if(text||S.pendingFiles.length){
      ...
      // /queue, /terminal, /goal, /yolo, /stop, /btw, or /background while the agent is running and have
      ...
      // /btw and /background dispatch their own side work while busy (#6597).
      if(text.startsWith('/')&&!literalSlash){
        const _pc=typeof parseCommand==='function'&&parseCommand(text);
        if(_pc&&['steer','interrupt','queue','terminal','goal','yolo','stop','btw','background'].includes(_pc.name)){

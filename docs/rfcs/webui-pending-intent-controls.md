...
- if delivery fails, preserves user input through Queue fallback or visible
  failure state
- remains attached to the immutable journal owner of the active run when Agent
  context compression rotates its writable session identity
- allocates, appends, and publishes the accepted control event in one per-run
  transaction, without consuming a replay sequence when the append fails

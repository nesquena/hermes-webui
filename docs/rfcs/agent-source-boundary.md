...
| Slash command registry and plugin commands | `hermes_cli.commands`, selected lightweight command helpers, and `hermes_cli.plugins` from `api/commands.py` | Command/plugin capability discovery API plus a WebUI-safe execution/resolve contract (`output` for status commands, `message` for agent-turn seeds) scoped by active profile | WebUI should render command help from a stable capability response and must not become a generic CLI passthrough. CLI-only, gateway lifecycle, TUI, clipboard, and upload/debug-takeover commands stay blocked or advisory. |

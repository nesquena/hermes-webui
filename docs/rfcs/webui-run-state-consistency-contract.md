...
| Model context / `context_messages` | Supplies conversation state to the agent | Must include the current visible user turn unless deliberately excluded with a user-visible reason; provider-facing repairs happen on independent outbound rows | Let the agent resume from context that contradicts what the user can see, mutate the visible transcript, or rewrite persisted history while repairing an outbound request |

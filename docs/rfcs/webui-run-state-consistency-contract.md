...
| Pending turn metadata | Bridges submitted-but-not-yet-finalized user input | Must identify the user turn and stream that own active work; a transcript timestamp is only a placement hint, not turn identity | Become a permanent duplicate transcript row after recovery or absorb a prior same-text turn with different attachments |

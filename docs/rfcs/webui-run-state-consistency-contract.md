...
| Run journal / replay | Rebuilds emitted runtime events after reconnect or restart | Must be cursor-safe and idempotent; server-emitted `state_saved` outcomes keep exact session/run/event/seq ownership when projected into Anchor side effects | Duplicate assistant text, thinking text, tool cards, compression cards, or durable side-effect records |

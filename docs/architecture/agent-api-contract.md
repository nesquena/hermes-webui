...
| `webui_local_or_client_package` | WebUI imports `hermes_cli.auth`, `hermes_cli.config`, `hermes_cli.plugins`, `hermes_cli.profiles`, `hermes_cli.goals`, `agent.skill_utils`, `agent.credential_pool`, and `hermes_constants`. `api/profiles.py` additionally resolves `hermes_cli.env_loader.hydrate_profile_secret_sources` lazily and optionally, to hydrate a profile's external secret sources (Bitwarden Secrets Manager, 1Password) into the profile runtime env; agents without the symbol degrade to a no-op. | Pure schemas, constants, and parsing helpers can move into a small versioned client/shared package. Privileged data such as credential pools, auth status, profile mutation, plugin discovery, goal persistence, and external secret-source hydration need hermes-agent endpoints. UI-only formatting can remain in WebUI. |

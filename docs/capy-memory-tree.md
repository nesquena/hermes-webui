...
The implemented refresh worker is intentionally narrow. It may ingest safe HTML/plain/Markdown, RSS/Atom, JSON Feed, and allow-listed GitHub API metadata only after the source origin is explicitly allowed. GitHub API refreshes are exact-shape parsers, not generic JSON fallbacks: supported metadata shapes include issue/PR (`/repos/{owner}/{repo}/{issues|pulls}/{number}`), repository (`/repos/{owner}/{repo}`), release (`/repos/{owner}/{repo}/releases/{id}`), branch (`/repos/{owner}/{repo}/branches/{branch}`), branch lists (`/repos/{owner}/{repo}/branches`), repository languages (`/repos/{owner}/{repo}/languages`), tag lists (`/repos/{owner}/{repo}/tags`), workflow (`/repos/{owner}/{repo}/actions/workflows/{workflow_id}`), workflow list (`/repos/{owner}/{repo}/actions/workflows`), workflow run (`/repos/{owner}/{repo}/actions/runs/{run_id}`), workflow jobs (`/repos/{owner}/{repo}/actions/runs/{run_id}/jobs`), check runs (`/repos/{owner}/{repo}/commits/{sha}/check-runs`), PR file lists (`/repos/{owner}/{repo}/pulls/{number}/files`), issue/PR comments (`/repos/{owner}/{repo}/{issues|pulls}/{number}/comments`), contributors (`/repos/{owner}/{repo}/contributors`), commit (`/repos/{owner}/{repo}/commits/{sha}`), and commit lists (`/repos/{owner}/{repo}/commits`). Branch summaries are reconstructed only from the branch name, protected flag, and commit SHA prefix; language summaries are reconstructed only from repository path, language count, total byte count, and the top safe language names/byte counts; check-run summaries are reconstructed only from repository path, commit SHA prefix, check-run counts, bounded ids/names/statuses/conclusions, and timestamps; contributor summaries are reconstructed only from safe logins and contribution counts; commit-list summaries are reconstructed only from repository path, commit count, SHA prefixes, first safe commit-message lines, author dates, and parent counts; commit URLs, protection details, check-run output/details/html URLs, check-run pull request rows, comment bodies, contributor avatar/user/profile URLs, commit-list avatar/html/API URLs, raw bodies, API-auth fields, prompts, scripts, and secret-looking values stay out of vault Markdown, search results, and public receipts. For `api.github.com/repos/...` origins whose path matches a supported family, malformed payloads fail closed instead of falling through to JSON Feed or generic JSON summaries.

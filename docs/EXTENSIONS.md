...
## TTS extension compatibility tombstone

Extensions can no longer register executable text-to-speech engines in the WebUI.
Provider discovery, provider configuration, credentials, and synthesis now belong
to the installed Hermes Agent and the active profile. The only selectable runtime
engines are **Browser** and **Hermes Agent**.

For compatibility with older extensions, this symbol remains defined:

```javascript
window.registerHermesTtsEngine(/* any value */); // always returns false
```

The function is a non-throwing tombstone. It does not retain descriptors or
callbacks, add a Settings option, make a network request, or synthesize audio.
Existing persisted extension IDs remain inert unavailable values so the UI can
show a repair choice without silently overwriting the user's saved setting.
Extension authors that need a new TTS provider should add it through the Hermes
Agent provider/plugin contract instead of injecting a WebUI audio transport.

...
On the Runs API path, WebUI does **not** currently send a `workspace` field on `POST /v1/runs`. Per-session workspace propagation was split out of the steer-relay work (#7440 gate review): the installed gateway's runs API does not read a `workspace` field, so sending one would be an inert no-op that silently misreports execution parity. Workspace propagation will land together with the agent-side consumer (Hermes Agent workspace-binding PR) and a capability advertisement the WebUI can negotiate against (`GET /v1/capabilities`), so the field is only ever sent to a gateway that provably consumes it; until then gateway tool execution uses the gateway's own working-directory behavior.

On the Runs API path, mid-run guidance typed into an active browser run is relayed as `POST /v1/runs/{run_id}/steer` with a JSON body `{"text": ...}`. Because the gateway publishes its run id only once the run has started, WebUI waits up to five seconds for that startup publication before sending. The relay resolves the gateway base URL from the same configuration authority the run was created with (the `webui_gateway_base_url` config key or `HERMES_WEBUI_GATEWAY_BASE_URL`), refuses HTTP redirects, and treats a response that does not verifiably land as a 2xx on the steer route itself as a delivery failure — a redirect to a 200 page is not proof of delivery. If the steer request is answered with HTTP 404, 405, or 410 — whether the endpoint is unsupported or the run is no longer known — WebUI preserves the submitted text in the next-turn queue and does not cancel the current run. For HTTP 409, other HTTP errors, timeouts, and network failures it restores the draft for manual retry instead, because an uncertain response may still have followed acceptance and automatic re-delivery could duplicate guidance. An accepted relay means the guidance was delivered to the gateway, not that the agent is proven to have applied it, and relaying never cancels the run. If the agent accepts the guidance but finishes its turn before consuming it, the terminal `run.completed` payload's `pending_steer` field is made durable server-side on the owning session (owner-scoped, keyed by the gateway run id) and also translated into the existing `pending_steer_leftover` event, so the browser queues the leftover text for the next turn exactly like the in-process backend instead of dropping it — and recovery no longer depends on a live SSE consumer being attached at completion time: any later load of the owning session (`GET /api/session`) re-offers an unconsumed leftover, and the slot is retired only by the turn that ships the text (a transactional ack on `/api/chat/start`, matched by run id) or by the user explicitly clearing the restored composer text. The steer-leftover queue entry is idempotent by run id, so replays and re-deliveries never duplicate guidance, and it is never discarded for preceding the final assistant timestamp (post-assistant user intent by construction). This relay requires the same `HERMES_WEBUI_GATEWAY_USE_RUNS_API=true` opt-in plus a gateway that advertises approval support; the legacy chat-completions transport has no run-id lifecycle to relay through, so mid-run guidance there degrades to the same next-turn queue (the run is never cancelled and nothing is claimed to have been applied live). The agent-side steer endpoint lives in Hermes Gateway, not in this repository, and is not verified here — this section describes WebUI's request behavior only and asserts no minimum agent version.

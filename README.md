...
- Boot deep links -- `?workspace=<path>` starts a fresh session bound to that workspace (one-shot: the parameter is removed from the URL once consumed, symmetric to `?profile=`); combine with `?q=<text>` to prefill the composer so external launchers can open the agent on the right project. The value is forwarded verbatim (surrounding whitespace is significant on Unix) and validated server-side (`resolve_trusted_workspace()`); if the path is rejected (the server tags that one `400` with `code: "invalid_workspace"`) the parameter is consumed and the boot falls back to restoring the saved session, while a transport failure (expired auth, network error, 5xx), a `400` raised by any other field, or a path the server merely could not inspect (permission denied -- grant access and reload) keeps it in the URL so the retry -- including the post-login bounce -- still carries the intent. With both `?profile=` and `?workspace=`, the workspace routing waits for the profile switch to complete and is retried on the next load if the switch fails. Combined with the `action=new-chat` app shortcut, the workspace routing owns the single session creation -- and while a workspace launch is still outstanding the boot holds: it shows the empty state rather than starting the shortcut chat or restoring a session, so the URL keeps carrying the intent and one launch always produces exactly one session.

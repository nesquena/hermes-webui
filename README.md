...
| `HERMES_WEBUI_HIDDEN_SKILLS` | *(unset)* | Comma- or whitespace-separated skill names hidden from the WebUI Skills panel, cron skill picker, and `/api/skills/usage` breakdown. Hidden skills stay on disk and remain usable by the underlying Hermes agent (CLI / cron / Telegram etc.) — this is a WebUI-only knob, useful for Docker / multi-tenant installs that want a curated skill set per container without forking the agent image. Matching is exact and case-insensitive against the SKILL.md frontmatter `name:` (or the containing directory name when no `name:` is set). Example: `HERMES_WEBUI_HIDDEN_SKILLS=imcrobot-skill, internal-admin` |

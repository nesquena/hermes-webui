...
- Pin/star sessions to the top of the sidebar (gold indicator). Pins are stored in hermes-agent's `state.db` (`sessions.pinned`), the same record Hermes Desktop and `hermes sessions pin` use, so a pin made in any of them shows in all of them; the WebUI's per-session sidecar only caches the flag and is overwritten from `state.db` on every sidebar build, independent of the *Show CLI sessions* setting. On upgrade, pins that exist only in sidecars are copied into `state.db` once per profile, verified and recorded in `_pin_state_db_migration.json` in the sessions directory, before `state.db` starts winning; a pinned conversation that compresses keeps its pin on the new segment. Pinning returns `503` if that record cannot be written, so a pin is never reported as saved when Desktop cannot see it. Sessions that exist only in the WebUI (no `state.db` row) keep their pin in the sidecar.

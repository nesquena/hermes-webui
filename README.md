...
| `HERMES_WEBUI_AGENT_CACHE_IDLE_TTL` | `3600` | Seconds a cached agent may sit idle before the cache governor evicts it. An evicted agent is committed and closed at the session boundary, so its warm prompt cache is lost and its next turn pays a cold reload. `0` disables this valve |
| `HERMES_WEBUI_AGENT_CACHE_MEMORY_HIGH_MB` | `auto` | Resident-memory budget for the pressure valve: when anonymous RSS crosses it, the governor soft-releases the transcripts of the least-recently-used cached agents — dropping `_session_messages` so the heap shrinks, while the agent and its cross-turn state stay warm and the transcript rebuilds from the persisted session on the next turn. A number is an explicit MB budget; `auto` derives it from the cgroup memory limit (falling back to total RAM); `0` (or `off`) disables this valve |
| `HERMES_WEBUI_AGENT_CACHE_PROTECT_RECENT` | `8` | Most-recently-used sessions the pressure valve never touches — their warm prompt cache is worth the most. Clamped to half the cache, so a handful of very large transcripts can never protect the entire LRU and leave the process with nothing it is willing to shed |
| `HERMES_WEBUI_AGENT_CACHE_GOVERN_INTERVAL` | `60` | Seconds between cache-governance passes (idle TTL + memory pressure). `0` disables the governor thread entirely rather than starting a busy-spin loop |

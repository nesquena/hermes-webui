...
def _load_repo_dotenv() -> None:
    ...
    Variables are loaded as defaults: an existing process environment value
    wins over the repo .env value. This keeps inline overrides such as
    ``HERMES_WEBUI_PORT=9000 hermes-webui web`` predictable.
...
def _agent_dir_from_hermes_cli() -> Path | None:
    """Resolve the agent import root by inspecting the `hermes` CLI.

    Supports package-manager wrappers that export HERMES_PYTHON and the
    installer-created console script whose shebang points into the agent venv.
    Standard filesystem candidates still win before this fallback runs.
    """
    discovered = agent_discovery.agent_dir_from_hermes_cli()
    if discovered:
        return discovered
...
def discover_agent_dir() -> Path | None:
    ...
    candidates = [
        ...
        str(xdg_data / "hermes-agent"),
        str(xdg_data / "hermes" / "hermes-agent"),
        str(xdg_data / "hermes-webui" / "hermes-agent"),
        str(xdg_state / "hermes-webui" / "hermes-agent"),
        str(Path.home() / ".hermes-webui" / "hermes-agent"),
...
def _python_can_run_webui_deps(python_exe: str) -> bool:
...
def ensure_python_has_webui_deps(python_exe: str, agent_dir: Path | None = None) -> str:
    ...
    if agent_dir is None:
        if _python_can_run_webui_deps(sys.executable):
            return sys.executable
        if _python_can_run_webui_deps(python_exe):
            return python_exe
    ...
    subprocess.run(
        [str(venv_python), "-m", "pip", "install", "--quiet", *install_args],
        check=True,
    )
    if agent_dir is None and _python_can_run_webui_deps(str(venv_python)):
        return str(venv_python)
    if _python_can_run_webui_and_agent(str(venv_python), agent_dir):
        return str(venv_python)
    if agent_dir is None:
        raise RuntimeError(
            "Python environment cannot import WebUI dependencies. Set "
            "HERMES_WEBUI_PYTHON to a Python environment with PyYAML installed "
            "or install the project with `python3 -m pip install -e .`."
        )
    raise RuntimeError(
...
def main(argv: list[str] | None = None) -> int:
    ...
    server_path = str(PACKAGE_ROOT / "server.py")

...
Capy Memory Tree source-refresh fetchers use endpoint-specific allow-listed parsers before generic JSON fallbacks. Current GitHub metadata parsers cover safe list/detail families such as releases/assets/reactions, labels/milestones/topics/languages, commits and pull-request/issue subresources, contributors/collaborators/teams, Actions workflows/runs/artifacts/caches/runners/permissions/workflow-run approvals, repository traffic/stats, Pages/deployments/environments/rulesets/branch protection, code/security/secret scanning summaries, and private-name/public-key metadata. These parsers must validate every row, persist only bounded metadata, and fail closed on bodies, raw URLs/query fragments, prompt-injection text, scripts, API-auth fields, renderer/source/data payloads, and secret-like values.

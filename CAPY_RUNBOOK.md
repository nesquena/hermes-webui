...
Space tool/adaptor boundaries that may be influenced by Memory Tree context return server-generated advisory/no-authority evidence rather than trusting request-supplied memory authority. Read, mutation, browser/navigation, package/shared-data, development, and recovery receipt paths should preserve `metadata_only`, `advisory_context`, `context_authority: untrusted_advisory`, `can_bypass_safety_gates: false`, required safety gates, forged-authority rejection, and safe output-compaction/progress receipts without echoing prompts, commands, renderer/source/data payloads, credentials, or generated bodies.

Demo catalog, demo-suite, and individual demo-smoke surfaces follow the same trust-envelope contract: backend receipts carry server-generated `memory_advisory`, `output_compaction`, and progress evidence, while UI cards render only allow-listed labels, counts, and required-gate metadata from those envelopes.

Capy Memory Tree source-refresh fetchers use endpoint-specific allow-listed parsers before generic JSON fallbacks. Current GitHub metadata parsers cover safe list/detail families such as README/license, releases/assets/reactions, labels/milestones/topics/languages, commits and pull-request/issue subresources, contributors/collaborators/teams, Actions workflows/runs/artifacts/caches/runners/permissions/workflow-run approvals, repository traffic/stats, Pages/deployments/environments/rulesets/branch protection, code/security/secret scanning summaries, dependency-graph SBOM metadata, Dependabot alert summaries, private-name/public-key metadata, GitHub environment deployment branch policies, and the exact GitHub Pages latest-build endpoint (`https://api.github.com/repos/{owner}/{repo}/pages/builds/latest`). These parsers must validate every row and final response URL against the exact sanitized canonical route before body parsing/persistence, persist only bounded metadata, and omit ignored GitHub advisory IDs, version ranges, latest-build raw URLs/pusher URL fields/error text, and raw URLs from persisted vault/search/public output. Source registration and source-refresh job receipts carry server-generated metadata-only/advisory/no-authority trust labels (`public_output: metadata_only`, `advisory_context`, `context_authority: untrusted_advisory`, `can_bypass_safety_gates: false`) that callers cannot spoof. Due-refresh requeues may preserve hidden canonical fetch origins for exact safe routes, but legacy rows with unsafe raw route candidates must fail closed to local `capy-memory://...` labels. JSON Feed/generic bypass attempts, final-response URL drift, route mismatches, malformed origins/payloads, prompt-injection text, scripts, API-auth fields, renderer/source/data payloads, and secret-like values must fail closed.

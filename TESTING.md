...
>
> Automated coverage: ~11,500 tests collected via `./scripts/test.sh tests/ --collect-only -q`. On every PR, GitHub Actions runs the full suite on Python 3.12 (5 parallel shards) plus a fast 3.11/3.13 compatibility job (dependency install, `compileall`, and `pytest --collect-only`); pushes to master execute the full suite on all three Python versions (15 shard jobs). This runs alongside a ruff lint gate, a headless browser smoke test, and a Docker smoke test. The suite covers the bootstrap/static wizard, real provider config persistence (`config.yaml` + `.env`), the `/api/onboarding/*` backend, the onboarding skip/existing-config guard, CSS regression coverage for thinking/tool card animation, streaming session persistence, mobile layout breakpoints, locale parity across 14 languages, and hundreds of issue/PR-pinned regression tests.
> Run: `./scripts/test.sh`
>

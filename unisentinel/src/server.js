...
async function main() {
  ...
  // helper to list alerts supporting both sync and async drivers
  const listAlerts = async (limit = 50) => {
    try {
      const res = db.listAlerts(limit);
      if (res && typeof res.then === 'function') return await res;
      return res;
    } catch (e) {
      console.warn('listAlerts failed', e && e.message);
      return [];
    }
  };
...
function parsePairsEnv(envStr) {
  ...
  async function seedDefaultAlerts() {
    const current = await listAlerts();
  ...
  app.get('/api/stats', async (req, res) => {
    const list = await listAlerts(50);
...
app.get('/api/alerts/:id/acknowledge', async (req, res) => {
  const list = await listAlerts(1000);
  const alert = list.find((entry) => entry.id === req.params.id);
  if (!alert) {
    return res.status(404).json({ error: 'alert not found' });
  }
  alert.acknowledged = true;
  // if db supports acknowledgeAlert, use it; otherwise fall back to file write
  if (typeof db.acknowledgeAlert === 'function') {
    try {
      await db.acknowledgeAlert(alert.id);
    } catch (e) {
      console.warn('acknowledgeAlert failed', e && e.message);
    }
    return res.json({ ok: true, alert });
  }

  const data = { alerts: list.filter((entry) => entry.id !== alert.id).concat(alert) };
  if (typeof db.write === 'function') db.write(data);

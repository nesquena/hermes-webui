...
- **Fix:** Added `_redaction_tolerant_match()` — a fallback comparison used by `_is_visible_output_echo` when the strict whitespace-compacted compare fails and the candidate carries a redaction mask (`***`). It folds whitespace, tokenizes both texts into a unit stream of boundary characters (`" \` , } ] ) : ; |` and space) and boundary-free fields, then requires a **1:1 unit alignment**: a redaction mask may stand for ≥1 character of exactly one field (the same scalar) and can never span a boundary into a neighbouring field, so a redaction-only difference counts as already-streamed while a genuinely different value (`"status":"started"` vs `"status":"completed"`) still differs. The single-pass alignment is linear in the visible length, so it is safe on the synchronous streaming callback (including the unbounded full-output substring path). Fixing the flag at the source corrects both the live-render and navigate-back replay paths. Regression tests in `tests/test_issue_progress_echo_dedupe.py`.

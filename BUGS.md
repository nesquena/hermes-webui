...
- **Fix:** Added `_redaction_tolerant_match()` — a fallback comparison used by `_is_visible_output_echo` when the strict whitespace-compacted compare fails and the candidate carries a redaction mask (`***`). It folds whitespace, then lexes both texts with an **escape-aware boundary tokenizer** into a unit stream of boundary characters (quote/backtick/apostrophe, `,` `{` `}` `[` `]` `(` `)` `:` `;` `|` `&` `=` `?`, and space) and boundary-free fields, and aligns the candidate's units against a contiguous run of visible units. A mask may stand for >=1 character of exactly one visible scalar; **quoted-mask alignment** lets a mask that constitutes an entire quoted scalar (`"***"`) consume the whole visible quoted region up to the matching close quote, so a real secret containing `;` `|` `,` `&` or a space still dedups. An unquoted mask can never span a separator, so a genuinely different value (`"status":"started"` vs `"status":"completed"`, or `token=***&status=ok` vs `token=sk-1&status=failed&status=ok`) still differs and real progress is never suppressed. Alignment is `O(visible x candidate)` in the worst case — **not** linear — so both inputs are hard-capped (`_ECHO_REDACTION_MAX_CAND_CHARS` / `_ECHO_REDACTION_MAX_VIS_CHARS`) and a total unit-comparison budget is enforced before/while aligning; rarest-literal anchor pruning keeps realistic inputs linear in the visible length. Every bound fails **closed** (no dedup), the safe direction: a false negative only re-appends a paragraph, while a false positive would destroy genuinely-new progress. That keeps the check safe to run inline on the synchronous streaming callback. Fixing the flag at the source corrects both the live-render and navigate-back replay paths. Regression tests in `tests/test_issue_progress_echo_dedupe.py`: a changed-status negative **plus** redaction-only positive matrix derived from `_ECHO_STRUCTURAL_CHARS | {" "}` (so coverage cannot drift from the production boundary set), quote-style/mask-placement/escaped-quote cases parameterized over both anchoring modes, at-cap vs cap+1 assertions for each hard bound with the comparison-budget fail-closed branch forced via monkeypatch, a revert-sensitive co-growing near-match growth oracle, and end-to-end `already_streamed` + journal-replay cases including a delimiter-bearing secret.

...
Last updated: 2026-06-08 CDT on branch `feat/capy-spaces-foundation`.

Current latest known completed code slice: the OpenHuman-inspired Capy Memory Tree / compaction / source freshness / policy / progress MVP surfaces are implemented as metadata-only foundations, with recent expansion through GitHub commit-comments source-refresh metadata, GitHub repository custom-properties source-refresh metadata, GitHub Actions caches source-refresh metadata, GitHub repository webhooks source-refresh metadata, GitHub security-advisories source-refresh metadata, GitHub community-profile source-refresh metadata, GitHub environments source-refresh metadata, GitHub contents source-refresh metadata, GitHub collaborators source-refresh metadata, GitHub traffic-clones source-refresh metadata, GitHub traffic-views source-refresh metadata, GitHub subscribers source-refresh metadata, GitHub code-frequency source-refresh metadata, GitHub participation-stats source-refresh metadata, GitHub latest-release source-refresh metadata, GitHub fork-list source-refresh metadata, GitHub commit-status source-refresh metadata, GitHub issue-list source-refresh metadata, GitHub pull-request list source-refresh metadata, GitHub pull-request requested-reviewers source-refresh metadata, GitHub pull-request commit-list source-refresh metadata, GitHub pull-request review-list source-refresh metadata, GitHub milestone-list source-refresh metadata, GitHub label-list source-refresh metadata, GitHub release-list source-refresh metadata, GitHub single-release hardening, GitHub release-assets source-refresh metadata, GitHub pull-request file-list source-refresh metadata, GitHub check-runs source-refresh metadata, GitHub Actions workflow-list source-refresh metadata, GitHub Actions workflow-run timing source-refresh metadata, GitHub repository Actions artifacts source-refresh metadata, GitHub Pages source-refresh metadata, source-style widget definition/blueprint preview/source/render output-compaction receipts, source-style widget patch/delete/bulk-delete/toggle output-compaction receipts, source-style and current widget read/list/see output-compaction receipts, source-style Space read/get output-compaction receipts, Space collection/current read output-compaction receipts, source-style Space open/reload navigation receipts, logical app URL helper receipts, logical storage path-helper receipts, recovery action/toggle/repair-list compaction receipts, template install/reset compaction/advisory receipts, package import/export compaction/advisory receipts, native widget mutation compaction receipts, widget SDK helper memory advisory receipts, collection/current-read advisory receipts, widget read/list advisory receipts, no-space SDK helper advisory receipts, path-helper advisory receipts, research-artifact/research-progress advisory receipts, delete-space advisory receipts, recovery-restore advisory receipts, and recovery-toggle advisory receipts.
...
1. Remaining prompt-preflight + advisory memory enforcement for high-risk recovery/tool boundaries; creator preview/commit, active-space instruction/context, direct Space update instruction writes, save-meta instruction writes, repair prompt, whole-Space and recovery-module repair-event list responses, source-style Space revision/history list responses, source-style layout repair, widget-runtime prompt queue/status UI, no-space widget SDK helpers, widget detail/read/get/see helpers, widget-runtime contract helper inspections, source-style widget definition/blueprint preview/source helpers, logical storage path helpers, receipt-only browser adapters, receipt-only development terminal/shell adapters with common payload pass/block classification, Space open/reload navigation helpers, source-refresh worker/manual refresh paths, camera stream prompt-preflight/action-policy/progress receipts, package export, Space checkpoint receipts, and recovery revision restore now have initial metadata-only receipts.

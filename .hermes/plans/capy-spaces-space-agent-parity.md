...
2. Product-visible compaction evidence for remaining long tool/subagent/browser/development/recovery outputs; run-all, individual demos, creator preview/commit, active context, Research Harness artifact completion, scoped progress, receipt-only Browser Surface, approved camera-stream, queued widget events, source-style Space create, widget recovery toggle, and recovery-module toggle receipts already use the first receipt pattern.

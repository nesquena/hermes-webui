...
Last updated: 2026-07-10 CDT on branch `feat/capy-spaces-foundation`.
...
Source toggle-widgets progress lifecycle slice (2026-07-10 01:33 CDT): the real multi-widget `space.spaces.togglewidgets` mutation now records `tool.started` before its mutation loop and `tool.completed` after all successful widget patches. The metadata-only response exposes both as `progress_events` under `layout.toggle:<space_id>`, preserves backward-compatible terminal `progress_event`, and threads both event types into output compaction without weakening prompt-preflight, supervised autonomy/model-route, Memory Tree advisory/no-authority, or hostile renderer/html/source/data/API-key/token omission. This backend-only slice does not change a UI surface.

Source rearrange-widgets progress lifecycle slice (2026-07-09 23:02 CDT): the real multi-widget `space.spaces.rearrangewidgets` mutation now records `tool.started` before its mutation loop and `tool.completed` after successful widget patches. The metadata-only response exposes both as `progress_events` under `layout.rearrange:<space_id>`, preserves backward-compatible terminal `progress_event`, and threads both event types into output compaction without weakening prompt-preflight, supervised autonomy/model-route, Memory Tree advisory/no-authority, or hostile renderer/source/html/data/API-auth/memory-authority/secret omission. This backend-only slice does not change a UI surface.

Session recovery repair-safe output-compaction metadata-only slice (2026-07-09 18:54 CDT): both successful and manual-review `repair_safe_session_recovery(...)` receipts now explicitly set `output_compaction["metadata_only"]` to `True`. Focused RED/GREEN regressions preserve the hostile/raw public-receipt omission contract for session IDs/paths/messages, backup paths, raw prompts, renderer/source/html/script/data/API-auth fields, credentials, bearer/access tokens, and secret-looking values without changing repair semantics or status codes.

Session recovery repair-safe model-route evidence alignment: `repair_safe_session_recovery(...)` now has explicit regression coverage for nested `model_route_resolution` evidence: `hint:reasoning`, safe resolved provider/model labels, and `metadata_only` / `local_only` routing evidence beside required prompt-preflight, an approval-gated supervised policy, the repair progress lifecycle, Memory Tree advisory/no-authority, and output-compaction receipts, while hostile/raw session, auth, prompt, renderer, source, token, and secret data remains omitted.

Session recovery audit model-route evidence alignment: the read-only audit/status trust-envelope regression now explicitly checks the nested `model_route_resolution` inside `audit_session_recovery(...)` autonomy policy: `hint:reasoning`, a safe resolved provider/model label, `metadata_only: true`, and `local_only: true` are present while prompt-preflight, non-persisted progress, Memory Tree advisory/no-authority, and output-compaction receipts still omit raw session paths, API-auth fields, bearer/token markers, renderer/source fields, raw prompts, and secret-looking values.
...
Session recovery audit full trust-envelope slice: `audit_session_recovery(...)` now keeps recovery audit/status reads fully read-only while returning required recovery-action prompt-preflight, a non-mutating supervised autonomy/model-route receipt with no approval gates, local-only non-persisted progress evidence, the server-generated Memory Tree advisory/no-authority envelope, and metadata-only output-compaction evidence. The focused regression covers hostile session directory, backup path, raw-prompt, renderer/source, API-auth, bearer/token, and secret-looking fixtures and proves those values stay out of public receipts while no progress JSONL row is created.

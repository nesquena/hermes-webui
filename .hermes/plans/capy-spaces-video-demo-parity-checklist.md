...
Created: 2026-04-27
Last implementation-status update: 2026-05-20
...
- **Autonomy mode visible:** **MVP implemented at product-home aggregate level, with creator preview, creator commit, and approved camera-stream per-action receipts implemented.** Creator preview/commit and camera stream actions now expose approval requirement, prompt-preflight status, model-route hint, and metadata/local-only flags where applicable; remaining high-risk source/tool boundaries still need equivalent per-action receipts.

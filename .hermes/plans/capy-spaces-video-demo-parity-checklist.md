...
- **Autonomy mode visible:** **MVP implemented at product-home aggregate level, with creator preview, creator commit, approved camera-stream, package-import tool-alias, package-export, manual Memory refresh, and recovery restore per-action receipts implemented.** Creator preview/commit, camera stream actions, package import aliases, package export, and manual source refresh now expose approval requirement, prompt-preflight status, model-route hint, and metadata/local-only flags where applicable; remaining high-risk source/tool boundaries still need equivalent per-action receipts.

...
- **Autonomy mode visible:** **MVP implemented at product-home aggregate level, with first creator-preview per-action receipt implemented.** Creator preview now shows `Supervised`/`Semi-autonomous`/`Autonomous` mode evidence, approval requirement, creator commit gates, prompt-preflight status, model-route hint, and metadata/local-only flags; remaining high-risk source/tool/commit boundaries still need equivalent per-action receipts.
- **Prompt-injection preflight visible:** **Creator-preview UI implemented; broader per-action coverage remains.** High-risk ingested source/context boundaries should show pass/warn/block status before context can influence creator-loop or agent actions.

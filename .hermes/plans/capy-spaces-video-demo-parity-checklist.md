...
- The Weather demo smoke now records the Prague weather observation and queues one safe `widget.refresh` event for the persistent weather widget, so the visible smoke status shows both prompt → widget metadata and the agent-mediated refresh bridge count.

...
- OpenHuman-inspired context-layer MVPs now exist as metadata-only Capy Spaces surfaces: local Memory Tree canonicalization/search/status, output compaction helper, source registry/freshness counts, Space detail/creator memory assist, product-home autonomy policy, product-home progress events, and Research Harness progress-event producer coverage. Remaining parity work should focus on automatic artifact ingestion, product-visible compaction evidence, safe source refresh workers, prompt-preflight enforcement, and broader creator/development/visual-QA progress producers.
...
4. Broader structured progress producers for real long-running creator/development/visual-QA tasks; Research Harness progress updates now emit the first workflow-level progress events.
...
- **Progress panel visible:** **MVP implemented at product-home aggregate level.** Research Harness progress updates now emit structured metadata-only events; long-running creator/development/visual-QA flows still need broader producer coverage.

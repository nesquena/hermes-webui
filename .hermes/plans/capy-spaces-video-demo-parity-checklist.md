...
- OpenHuman-inspired context-layer MVPs now exist as metadata-only Capy Spaces surfaces: local Memory Tree canonicalization/search/status, output compaction helper, source registry/freshness counts, local knowledge source registration, automatic Space artifact ingestion at manifest/revision/rollback/repair/widget-event/visual-QA boundaries, Space detail/creator memory assist, product-home autonomy policy, creator preview/commit per-action autonomy and prompt-preflight receipts, product-home plus Space-detail progress events, Research Harness progress-event producer coverage, creator visual-QA commit progress events, Memory Tree source-refresh ingest progress events, demo-suite run progress events, and source-style layout-repair progress events. Remaining parity work should focus on prompt-preflight enforcement for source/context boundaries, product-visible compaction evidence, safe source-refresh scheduling/fetcher breadth, and broader browser/development/repair progress producers.
...
4. Broader structured progress producers for real long-running browser/development/repair flows; Research Harness progress updates, creator visual-QA commit gates, Memory Tree source-refresh ingest workers, demo-suite run events, and source-style layout-repair actions now emit the first workflow/gate/ingest/run/repair progress events and Space detail can inspect Space-scoped streams.
...
- **Prompt-injection preflight visible:** **Creator-preview/commit receipt path implemented; broader per-action coverage remains.** High-risk ingested source/context boundaries should show pass/warn/block status before context can influence creator-loop or agent actions.

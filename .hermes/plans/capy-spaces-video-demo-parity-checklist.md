...
4. Broader structured progress producers for real long-running browser/development/repair flows; Research Harness progress updates, creator preview receipts, creator visual-QA commit gates, Memory Tree source-refresh ingest workers, demo-suite run events, individual Browser Surface smoke events, source-style layout-repair actions, shared data slot set/delete actions, source-style widget upsert/renderWidget/blueprint preview actions, recovery widget enable/disable toggles, recovery revision restores, source-style Space deletions, and sanitized Space Agent package import/export operations now emit the first workflow/gate/ingest/run/browser/repair/cooperation/construction/recovery/package progress events and Space detail can inspect Space-scoped streams.

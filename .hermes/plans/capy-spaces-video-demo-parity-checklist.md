...
Created: 2026-04-27
Last implementation-status update: 2026-05-25
...
- OpenHuman-inspired context-layer MVPs now exist as metadata-only Capy Spaces surfaces:
  - Memory/source surfaces: local Memory Tree canonicalization/search/status, source registry/freshness counts, safe source-refresh worker, local knowledge source registration, and automatic Space artifact ingestion at manifest/revision/rollback/repair/widget-event/visual-QA boundaries.
...
- Source-style space removal aliases now include metadata-only `space.spaces.removeSpace` / `deleteSpace`, accepting Space Agent-style `spaceId` while deleting through Capy's revisioned primitive and returning bounded metadata plus action-policy/progress receipts.
- Source-style space duplication aliases now include metadata-only `space.spaces.duplicateSpace` / `cloneSpace`, accepting Space Agent-style `spaceId` while copying only safe Space metadata and widget summaries into a new persisted Space and returning autonomy-policy plus progress receipts for the creation boundary.
...
- Source-style logical storage path helper coverage now includes metadata-only Action policy and structured progress receipts for `space.spaces.buildSpace*Path`, exposing destructive-external-action approval, `hint:fast` route evidence, and safe `path.helper:<space_id>` run ids without echoing renderer/source/html/API-auth/script/token/secret payload markers.
...
2. Broader compaction producers beyond the implemented run-all, individual-demo, creator preview/commit, active-context, and scoped-progress receipts for long tool/subagent/browser/development/recovery outputs.
3. Broader source-refresh scheduler/cron coverage and source-specific fetchers now that the safe metadata-only refresh worker, manual trigger path, and local knowledge bridge exist.

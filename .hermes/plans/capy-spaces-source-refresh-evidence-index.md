...
Last count refresh: 2026-07-05 08:22 CDT
...
Snapshot generated from the checked-out `tests/test_capy_memory_tree.py` on 2026-07-05 08:22 CDT:
...
| `def test_.*before_body_read` | 127 | Regressions that explicitly name no-body-read behavior. |
| `def test_.*relevant_memory_empty` | 111 | Regressions that also prove Spaces relevant-memory remains empty after hostile drift. |

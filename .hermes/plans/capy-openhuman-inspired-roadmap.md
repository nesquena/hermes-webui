...
- Active-space context receipts, repair prompts and empty-prompt whole-Space/widget/module repair queues, widget-runtime prompts (including direct sandbox queue-status UI receipts), widget-runtime contract helper receipts, fixed widget reload/refresh events without free-form prompts, source-style widget definition/blueprint preview/source helpers, and source-refresh ingestion paths now use protected prompt-preflight boundaries before advisory context or user-provided instructions can influence an agent/tool action.

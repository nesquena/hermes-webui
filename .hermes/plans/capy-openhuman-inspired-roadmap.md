...
- Receipt-only development terminal/shell tool actions now return metadata-only required prompt-preflight, autonomy-policy, `development.terminal:<space_id>` progress, and output-compaction receipts without executing commands, enabling filesystem writes, or storing raw command/prompt/source/html/renderer/API-auth payloads.

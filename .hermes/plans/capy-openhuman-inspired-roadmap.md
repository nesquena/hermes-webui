...
- Recovery-module repair-event list responses now emit metadata-only `tool.completed` progress receipts with safe `recovery.module.repair_events:recovery-modules` run ids and thread that run id into the compaction evidence beside prompt-preflight/action-policy status, without exposing module source, renderer, prompt text, session ids, bearer strings, or secret-looking fixture values.

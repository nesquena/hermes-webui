...
- GitHub Actions workflow source-refresh metadata now has a narrow allow-listed parser for `/repos/{owner}/{repo}/actions/workflows/{workflow_id}` API payloads, reconstructing summaries only from workflow id, name, state, and timestamps while ignoring workflow paths/YAML/jobs, URLs, body fields, API-auth fields, scripts, prompts, and secret-like markers.
...
Remaining:
- Broaden cron/daemon triggering and additional source-specific fetchers while keeping existing manual/scheduled UI trigger receipts, GitHub issue/PR/repository/release/branch/tag/workflow/workflow-run/workflow-jobs/commit metadata ingestion, and all public UI metadata-only.

...
- Source-style widget upsert and renderWidget tool actions now emit metadata-only `tool.completed` progress events with safe `widget.upsert:<space_id>` and `widget.render:<space_id>` run ids, so agent-driven Space construction and generated-widget quarantine/render boundaries appear in Space-scoped progress without storing widget bodies, renderer/source/API-auth fields, prompts, scripts, or credentials.

...
class TestSettledRendererRepairBatch:
    ...
    def test_closed_suffix_malformed_link_scan_has_bounded_growth(self, driver_path):
        script = r"""
const fs=require('fs'); const src=fs.readFileSync(process.argv[1],'utf8');
global.window={}; global.document={createElement:()=>({innerHTML:'',textContent:''}),baseURI:'http://localhost/app/'};
function _sessionUrlForSid(sid){return '/app/session/'+encodeURIComponent(String(sid||''));}
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const _IMAGE_EXTS=/\.(png|jpg|jpeg|gif|webp|bmp|ico|avif)$/i,_SVG_EXTS=/\.svg$/i,_AUDIO_EXTS=/\.(mp3|ogg|wav|m4a|aac|flac|wma|opus|webm)$/i,_VIDEO_EXTS=/\.(mp4|webm|mkv|mov|avi|ogv|m4v)$/i;
function _inlineMediaHtmlForRef(ref){return String(ref||'');}
function extractFunc(name){const re=new RegExp('function\\s+'+name+'\\s*\\(');const start=src.search(re);if(start<0)throw new Error(name);let i=src.indexOf('{',start),d=1;i++;while(d>0&&i<src.length){if(src[i]==='{')d++;else if(src[i]==='}')d--;i++;}return src.slice(start,i);}
eval(extractFunc('_matchBacktickFenceLine')); eval(extractFunc('_isBacktickFenceClose')); eval(extractFunc('_normalizeMarkdownLinkDestination')); eval(extractFunc('renderMd'));
const ns=[4096,8192]; const medians=ns.map(n=>{const input='[x]('.repeat(n)+')'.repeat(n),a=[];for(let k=0;k<3;k++){const t=process.hrtime.bigint();renderMd(input);a.push(Number(process.hrtime.bigint()-t)/1e6);}a.sort((x,y)=>x-y);return a[1];}); process.stdout.write(JSON.stringify({medians}));
"""
        result = subprocess.run([NODE, '-e', script, str(UI_JS_PATH)], capture_output=True, text=True, timeout=30, check=False)
        assert result.returncode == 0, result.stderr
        small, large = json.loads(result.stdout)["medians"]
        assert large < 250.0, f"closed-suffix malformed scan exceeded 250 ms: {small:.3f}/{large:.3f}"
        assert large <= 3.0 * small, f"closed-suffix malformed scan grew superlinearly: {small:.3f}/{large:.3f}"
    ...
    def test_raw_anchor_preserves_non_link_inline_formatting(self, driver_path):
        markdown = '<a href="https://outer.example/path">**Bold** and ![Alt](https://img.example/a.png)</a>'
        out = _render(driver_path, markdown)
        assert len(re.findall(r"<a(?:\s|>)", out)) == 1, out
        assert 'href="https://outer.example/path"' in out, out
        assert '<strong>Bold</strong>' in out, out
        assert '<img' in out and 'https://img.example/a.png' in out, out
        assert 'href="https://img.example/a.png"' not in out, out

...
def test_polling_payloads_are_deferred_while_user_scrolls_sidebar():
    ...
    assert "_pendingSessionListPayload={gen:_gen,sessData,projData,unreadGen,requestAllProfiles};" in refresh_block
    assert "_schedulePendingSessionListApply();" in refresh_block
    assert "_applySessionListPayload(sessData,projData,{unreadGen,requestAllProfiles});" in refresh_block

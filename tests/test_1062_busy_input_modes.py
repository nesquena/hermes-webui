...
class TestSlashCommandHandlers:
    ...
    def test_cmd_steer_delegates_to_try_steer(self):
        ...
        # cmdSteer delegates to _trySteer; fallback must queue without cancelling.
        assert "_trySteer" in body, "cmdSteer must call _trySteer to use the real /api/chat/steer endpoint"
        # The shared helper must contain the non-destructive queue-only fallback path.
...
# ── send() busy branch ───────────────────────────────────────────────────
    ...
    def test_slash_commands_clear_pending_files(self):
        """Queue/interrupt clear S.pendingFiles after enqueuing; steer fallback
        queues staged files without interrupting the active stream.

        cmdQueue and cmdInterrupt call queueSessionMessage themselves and clear
        S.pendingFiles directly. cmdSteer delegates to _trySteer. _trySteer
        also clears files after queue-only fallback because the files were copied
        into queueSessionMessage.
        """
        ...
        # cmdSteer delegates to _trySteer; the helper clears files after copying
        # them into the queued next-turn payload.
        idx_try = COMMANDS_JS.find("function _trySteer(")
        assert idx_try >= 0, "_trySteer not found"
        try_body = COMMANDS_JS[idx_try:idx_try + 1800]
        assert "queueSessionMessage" in try_body
        assert "S.pendingFiles=[]" in try_body
        assert "renderTray()" in try_body
        assert "cancelStream" not in try_body
...
class TestSendBusyBranchDispatch:
    ...
    def test_send_busy_steer_preserves_files_when_steer_not_delivered(self):
        """Busy-mode steer queues staged files without interrupting on fallback.

        A failed steer leaves the active stream running; _trySteer copies staged
        files into the queue-only fallback payload and clears the tray.
        """
        send_idx = MESSAGES_JS.find("async function send(")
        ...
        assert "S.pendingFiles=[];renderTray();" in branch
        assert branch.index("await _trySteer(text, /*explicitSteer=*/false);") < branch.index("S.pendingFiles=[];renderTray();")

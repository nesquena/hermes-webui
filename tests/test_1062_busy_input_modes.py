...
class TestSlashCommandHandlers:
    ...
    def test_cmd_interrupt_delegates_to_try_interrupt(self):
        idx = COMMANDS_JS.find("async function cmdInterrupt(")
        assert idx >= 0
        body = _source_between(COMMANDS_JS, "async function cmdInterrupt(", "\n/**\n * /steer <message>")
        assert "_tryInterrupt" in body, "/interrupt must delegate to the shared redirect-first helper"
...
# ── send() busy branch ───────────────────────────────────────────────────
    ...
    def test_slash_commands_clear_pending_files(self):
        ...
        cmdQueue clears S.pendingFiles directly. cmdInterrupt and cmdSteer
        delegate to shared helpers. _trySteer no
        ...
        assert "if(!originalMsg&&pendingFilesSnapshot.length===0)" in interrupt_body, (
            "interrupt helper should allow file-only input through without redirect/cancel fallback"
        )
        assert "if(!pendingFilesSnapshot.length && ownerStreamId)" in interrupt_body
        queue_idx = interrupt_body.find("queueSessionMessage(")
        assert interrupt_body.find("/api/chat/interrupt") < queue_idx
        assert interrupt_body.find("return true") < queue_idx < interrupt_body.find("cancelStream", queue_idx)
        assert "if(_ownerStreamIsCurrent(ownerSid,ownerStreamId)&&typeof cancelStream==='function')" in interrupt_body
        assert "S.pendingFiles=[]" in interrupt_body

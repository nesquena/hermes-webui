...
def _join_ops_runner_threads(timeout=5.0):
    """Wait for any in-flight ops runner thread to finish.

    `start_action` spawns a daemon thread that releases `_ACTION_LOCK` in its
    `finally`. Tests that deliberately leave a process stuck (single-flight
    coverage) therefore leave that thread alive past the end of the test. If
    the next test then acquires the lock, the straggler's release hands the
    slot away underneath it — an intermittent failure in whichever test runs
    next, not in the one that caused it. Joining first makes the reset
    deterministic instead of racing it.
    """
    deadline = time.time() + timeout
    for thread in list(threading.enumerate()):
        if not str(thread.name).startswith("hermes-webui-ops-"):
            continue
...
def _reset_ops_actions_state():
    ...
    def _reset():
        ...
        # call even if a previous test's background thread left it held. It is
        # a last resort: the join above is what normally clears the slot.
...
def _panels_source():
    ...
    return (Path(__file__).resolve().parents[1] / "static" / "panels.js").read_text(
        encoding="utf-8"
    )


def test_ops_poll_is_cancelled_on_every_settings_teardown_path():
    """Closing the panel is a teardown path too, not just switching section.

    A 2-second poll chain that outlives the card it renders into keeps hitting
    /api/ops/status for a surface the user has left.
    """
    panels = _panels_source()

    switch_idx = panels.index("switchSettingsSection=function(name, opts){")
    switch_body = panels[switch_idx:panels.index("\n};", switch_idx)]
    assert "_stopOpsActionsPolling()" in switch_body, (
        "leaving the System section must cancel the poll"
    )

    hide_idx = panels.index("function _hideSettingsPanel(){")
    hide_body = panels[hide_idx:panels.index("\n}", hide_idx)]
    assert "_stopOpsActionsPolling" in hide_body, (
        "closing the settings panel must cancel the poll"
    )


def test_ops_poll_is_self_scheduling_and_guarded():
    """No `setInterval`: one request armed only after the prior one settles."""
    panels = _panels_source()

    sync_idx = panels.index("function _syncOpsActionsPolling(data){")
    sync_body = panels[sync_idx:panels.index("\n}", sync_idx)]
    assert "setInterval" not in sync_body
    assert "setTimeout" in sync_body
    assert "_opsActionsPollInFlight" in sync_body

    load_idx = panels.index("function loadOpsActionsStatus(){")
    load_body = panels[load_idx:panels.index("\n}\nasync function runOpsAction", load_idx)]
    assert "if(_opsActionsPollInFlight) return;" in load_body, (
        "a slow status request must not fan out into concurrent polls"
    )
    assert "_stopOpsActionsPolling()" in load_body, (
        "a failed poll must stop the chain instead of retrying forever"
    )


def test_backup_confirmation_states_what_the_archive_contains():
    """The user must be told the ZIP carries .env / auth material."""
    panels = _panels_source()

    run_idx = panels.index("async function runOpsAction(action){")
    run_body = panels[run_idx:panels.index("\n}\nfunction downloadOpsBackup", run_idx)]
    assert "ops_confirm_backup_contents" in run_body
    ...
    en_start = i18n.index("\n  en: {")
    en_block = i18n[en_start:i18n.index("\n  it: {", en_start)]
    contents_line = next(
        line for line in en_block.splitlines()
        if line.strip().startswith("ops_confirm_backup_contents:")
    )
    lowered = contents_line.lower()
    assert ".env" in lowered
    assert "auth" in lowered or "credential" in lowered


def test_backup_download_uses_a_temporary_anchor_not_window_open():
    """`window.open` is popup-blocked and unreliable in the iOS PWA."""
    panels = _panels_source()

    idx = panels.index("function downloadOpsBackup(){")
    body = panels[idx:panels.index("\n}", idx)]
    # Match the CALL, not the word: the comment explains why window.open is
    # not used, and a substring check would flag its own rationale.
    assert "window.open(" not in body
    assert "createElement('a')" in body
    assert "a.click()" in body


def test_ops_log_details_are_updated_in_place_not_rebuilt():
    """Rebuilding the card every poll dropped <details> open state and scroll."""
    panels = _panels_source()

    idx = panels.index("function _renderOpsActionsCard(data){")
    body = panels[idx:panels.index("\nfunction _stopOpsActionsPolling", idx)]
    assert "_opsActionsCardShape" in body
    assert "data-ops-log-details" in body
    assert 'aria-live="polite"' in body, "status changes need a live region"
    assert "pre.scrollTop" in body, "scroll position must survive a poll"
...
def test_profile_env_cannot_open_the_ops_gate(monkeypatch):
    """Gate review P0: the gate decision is an immutable startup snapshot —
    a per-profile environment overlay that lands in the LIVE os.environ after
    startup must not enable status details, download, or start."""
    monkeypatch.setattr(routes, "_OPS_ACTIONS_GATE_STARTUP_SNAPSHOT", False)
    # Stands in for the per-profile environment overlay applied on switch:
...
def test_terminal_status_implies_the_slot_is_already_free(monkeypatch, tmp_path):
    """`status != running` is the signal the UI acts on — it must not lie.

    Post-processing (chmod, archive validation, retention sweep) runs while
    the single-flight lock is still held. Publishing "completed" before that
    finished made the card re-enable its buttons a moment early: a click
    landing in that window got a spurious 409 "another action is running",
    and the Download affordance was still missing because validation had not
    run yet. The terminal status is now published only after the slot is free
    and the archive record is in place.
    """
    from api import ops_actions
    ...
    # Everything the terminal status promises must already be true.
    assert ops_actions._ACTION_LOCK.locked() is False, (
        "terminal status was published while the single-flight slot was held"
    )
    assert status["backup_path"] == str(archive)
    assert status["backup_available"] is True

    # And a new action really can start immediately.
    monkeypatch.setattr(
        ops_actions.subprocess, "Popen", _make_fake_popen(["ok\n"], returncode=0)
    )
    started, _snapshot = ops_actions.start_action("doctor", "default")
    assert started, "a follow-up action was rejected right after a terminal status"
    _wait_until_not_running()


class _SlotBarrierLock:
    """Single-flight lock wrapper that parks ONE releasing thread.

    The park happens inside `release()`, i.e. at exactly the window the
    generation race lives in: the slot is already free for the next run, but
    the releasing run has not published its terminal state yet.
    """
    ...
    def release(self):
        ...
        if self._armed:
            self._armed = False
            self.parked.set()
            self.resume.wait(timeout=15)

    def locked(self):
        return self._inner.locked()


def _archive_writing_popen(archives: list):
    """Fake Popen that writes one uniquely-marked ZIP per backup run."""

    def _on_spawn(cmd, kwargs):
        if "--output" not in cmd:
            return  # doctor / security audit produce no artifact
        out = Path(cmd[cmd.index("--output") + 1])
        import zipfile as _zf

        marker = f"run-{len(archives)}"
        with _zf.ZipFile(out, "w") as zf:
            zf.writestr(marker, marker)
        archives.append(out)

    return _make_fake_popen(["ok\n"], returncode=0, on_spawn=_on_spawn)


def _park_a_backup_at_the_release_window(monkeypatch, tmp_path, archives):
    """Start backup A and block it between slot release and publication."""
    from api import config, ops_actions, profiles
    ...
    monkeypatch.setattr(config, "STATE_DIR", tmp_path / "webui_state")
    monkeypatch.setattr(
        ops_actions.subprocess, "Popen", _archive_writing_popen(archives)
    )

    barrier = _SlotBarrierLock(ops_actions._ACTION_LOCK)
    monkeypatch.setattr(ops_actions, "_ACTION_LOCK", barrier)

    started, _ = ops_actions.start_action("backup", "default")
    assert started
    assert barrier.parked.wait(timeout=10), "backup A never reached the release window"
    assert archives, "backup A produced no archive"
    return barrier


def test_older_backup_cannot_roll_back_a_newer_runs_backup_pointer(
    monkeypatch, tmp_path
):
    """Generation race: the validated-archive pointer belongs to the run that
    owns the slot.

    Barrier: backup A is parked the instant the slot is released and before
    its terminal state is published. Backup B takes the freed slot and runs to
    completion. When A resumes, `last_successful_backup`, the status snapshot
    and the download must all still resolve to B's newer archive — assigning
    the pointer after the release let A silently roll it back to its own,
    older archive.
    """
    from api import ops_actions

    archives: list[Path] = []
    barrier = _park_a_backup_at_the_release_window(monkeypatch, tmp_path, archives)
    old_archive = archives[0]

    started_b, _ = ops_actions.start_action("backup", "default")
    assert started_b, "the slot was not actually free while A was parked"
    final_b = _wait_until_not_running(timeout=10)
    assert final_b["status"] == "completed"
    assert len(archives) == 2
    new_archive = archives[1]
    ...
    status = ops_actions.get_status("default")
    assert status["last_successful_backup"] == str(new_archive.resolve()), (
        "the older run rolled the latest-backup pointer back to its own archive"
    )
    assert status["backup_path"] == str(new_archive.resolve())
    assert status["backup_available"] is True
    assert ops_actions.latest_backup_path("default") == new_archive.resolve()
    ...
    assert _zf.ZipFile(_io.BytesIO(bytes(dl.body))).namelist() == ["run-1"], (
        "download served the older run's archive"
    )


def test_parked_backup_stays_latest_without_contaminating_a_doctor_run(
    monkeypatch, tmp_path
):
    """Same barrier, B = doctor: A is still the newest archive afterwards, and
    none of A's run-scoped fields may leak into the doctor run's state."""
    from api import ops_actions
    ...
    started_b, _ = ops_actions.start_action("doctor", "default")
    assert started_b, "the slot was not actually free while A was parked"
    final_b = _wait_until_not_running(timeout=10)
    assert final_b["status"] == "completed"
    ...
    status = ops_actions.get_status("default")
    assert status["action"] == "doctor"
    assert status["status"] == "completed"
    assert status["backup_path"] is None, "A's run-scoped archive leaked into B"
    assert status["last_successful_backup"] == str(archive.resolve())
    assert ops_actions.latest_backup_path("default") == archive.resolve()


def test_timeout_slot_stays_closed_until_a_sigterm_ignoring_child_is_gone(
    monkeypatch, tmp_path
):
    """Reaping the leader is not proof the owned tree is gone.

    Leader exits during the SIGTERM grace period, so the SIGKILL escalation
    is never reached — while a child that ignores SIGTERM keeps running. The
    slot may not reopen (and the terminal status may not be published) until
    that child is provably gone.
    """
    from api import config, ops_actions, profiles
    ...
    monkeypatch.setattr(config, "STATE_DIR", tmp_path / "webui_state")
    monkeypatch.setattr(ops_actions, "_ACTION_TIMEOUT_SECONDS", 1)
    monkeypatch.setattr(ops_actions, "_TERM_GRACE_SECONDS", 5)
    ...
    # `exec sleep` makes the leader die instantly on the group SIGTERM; the
    # child stays in the same process group and ignores it.
    monkeypatch.setattr(
        ops_actions,
        "_command_for_action",
        lambda action, backup_dir: [
            "bash",
            "-c",
            f"bash {child_script} & exec sleep 300",
        ],
    )
    ...
    while time.time() < deadline:
        try:
            ...
            time.sleep(0.05)
    assert child, "child pid was never written"

    try:
        final = _wait_until_not_running(timeout=30)
        assert final["status"] == "timeout"
        # No polling grace on purpose: the terminal status is the signal the
        # UI acts on, so it may only appear once the tree is already gone.

...
def test_rotation_lineage_lookup_does_not_hold_stream_registry_lock(
    ...
    def match_lineage(candidate, requested_sid):
        acquired = STREAMS_LOCK.acquire(blocking=False)
        lock_states.append(acquired)
        if acquired:
            STREAMS_LOCK.release()
        return candidate is agent and requested_sid == continuation_sid
...
def test_compressed_continuation_uses_immutable_stream_journal_owner(
    ...
    assert not (tmp_path / "run_journal" / new_sid / f"{stream_id}.jsonl").exists()
...
def test_two_steers_and_teardown_do_not_form_stream_owner_journal_lock_cycle(
    ...
    teardown_holds_streams = threading.Event()
    teardown_done = threading.Event()
    results = []
    ...
    stream = Stream()
    writer = run_journal.RunJournalWriter(sid, stream_id, session_dir=tmp_path)
    monkeypatch.setattr(streaming, "STREAM_SESSION_OWNERS_LOCK", owner_lock)
    monkeypatch.setattr(streaming, "STREAM_SESSION_OWNERS", owners)
    monkeypatch.setattr(streaming, "RunJournalWriter", lambda *_args, **_kwargs: writer)

    def peek_stream(_stream_id):
        with streams_lock:
            return stream if stream_active.get(stream_id) else None

    monkeypatch.setattr(streaming, "peek_stream", peek_stream)
    ...
    while not owner_lock.locked() and time.time() < deadline:
        time.sleep(0.01)
    assert owner_lock.locked(), "second steer did not hold the owner lock while waiting for the journal"
    teardown_thread.start()
    assert teardown_holds_streams.wait(timeout=5)
    ...
    assert not second.is_alive(), "second steer deadlocked waiting for the journal"
    assert teardown_done.is_set(), "teardown deadlocked waiting for the owner registry"
    assert len(results) == 2
    assert all(result == (True, None, True) for result in results)
    assert sorted(item[1]["text"] for item in published) == ["first", "second"]
...
def test_missing_registered_owner_after_liveness_check_rejects_steer(
    ...
    assert not (tmp_path / "run_journal" / continuation_sid / f"{stream_id}.jsonl").exists()

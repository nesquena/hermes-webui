...
PR(b) ring-buffer dedupe. The insertion order is contractual:
  ...
  4. T4 ``_isSessionActivelyViewed(sid)`` early return (NEW)
  5. ``showToast(...)``                      (NEW per Q-c-1)
  6. diagnostic ack POST                    (existing)
...
uses. Each grep is precise so a behavioural regression (e.g. moving the toast
before the focus gate, or leaking ``d.command`` / ``d.exit_code`` into the
...
def _focus_gate_match(body: str):
    """Locate the T4 early-return — either ``if (_isSessionActivelyViewed(sid)) return;``
    direct form, OR the indirect form via a ``_viewed`` flag that is itself
    set from ``_isSessionActivelyViewed(sid)``. Returns an object exposing
    ``.start()`` / ``.end()`` of the early-return statement (start = condition
    head, end = closing brace of the gate block / end of bare return)."""
    # Direct form.
    m = re.search(
    ...
    # if (_viewed) { ... return; ... }  — brace-balance walk to find the gate block end.
    flag_decl = re.search(
    ...
    if close_abs is None:
        return None
    # Confirm a `return` lives inside the block.
    if "return" not in body[open_abs:close_abs]:
        return None
...
def test_focus_gate_is_after_ring_buffer_dedup():
    """T4 ``_isSessionActivelyViewed(sid)`` early-return MUST appear AFTER the
    ...
    assert gate_match is not None, "T4 focus gate (early return) missing"
    assert gate_match.start() > dedupe_idx, (
        "T4 focus gate must follow the ring-buffer dedup check"
    )


def test_mark_as_seen_is_after_dedup_and_before_focus_gate_return():
    """Mark-as-seen + clear-unread bookkeeping MUST run between dedup and the
    T4 early-return statement so an actively-viewed session clears its unread
    counter even though the toast is suppressed. The bookkeeping may sit
    inside the same ``if (_viewed)`` block as the early-return, so we assert
    bookkeeping ends BEFORE the early-return token itself."""
    body = _handler_body()
    dedupe_idx = body.index("_bgTaskCompleteRingBufferAdd(sid, evt_id)")
    ...
    # Locate the actual `return;` that owns the T4 short-circuit.
    return_re = re.search(r"return\s*;\s*//\s*T4", body)
    if return_re is None:
        # Fallback: first `return` inside the gate match span.
        return_re = re.search(r"return\s*;", body[gate_match.start() : gate_match.end()])
        assert return_re is not None
        return_pos = gate_match.start() + return_re.start()
    else:
        return_pos = return_re.start()
    assert dedupe_idx < mark_idx < return_pos, (
        "_markSessionViewed must sit between dedupe and the T4 early return"
    )
    assert dedupe_idx < clear_idx < return_pos, (
        "_clearSessionCompletionUnread must sit between dedupe and the T4 early return"
    )


def test_toast_call_is_after_focus_gate():
    """The ``showToast`` call MUST come after the T4 early return so a
    focused viewer never sees a toast for the session they are watching."""
    body = _handler_body()
    gate_match = _focus_gate_match(body)
    assert gate_match is not None
    toast_idx = body.index("showToast(")
    assert toast_idx > gate_match.end(), (
        "showToast must appear after the T4 drop-when-focused early return"
    )


def test_toast_call_is_before_ack_post():
    """The toast hunk lands BEFORE the diagnostic ack POST so the user-facing
    surface fires even if the ack is somehow rejected by the network."""
    body = _handler_body()
    toast_idx = body.index("showToast(")
    ack_idx = body.index("api/bg-task-complete-ack")
    assert toast_idx < ack_idx, "showToast must precede the diagnostic ack POST"

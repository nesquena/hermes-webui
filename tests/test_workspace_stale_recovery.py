...
def test_chat_start_rechecks_profile_after_workspace_owner_adoption(
    ...
    # The adopted durable owner belongs to a known other profile, so the
    # post-adoption recheck uses the same #7710 contract as the first check.
    assert response["_status"] == 409
    assert response["code"] == "session_profile_mismatch"
    assert response["profile"] == "other"

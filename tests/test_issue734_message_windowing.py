...
def test_message_virtualization_switches_render_messages_to_scroll_driven_window():
    ...
    assert "if(virtualWindow.virtualized&&renderTailStart>windowEnd&&vi===headRenderCount)" in UI_JS
...
def test_load_earlier_only_pages_server_history_and_preserves_scroll():
    ...
    assert "renderMessages({preserveScroll:true, _prependAnchor:viewportAnchor, _ownedPrepend:true});" in SESSIONS_JS

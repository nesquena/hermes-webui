...
if (!/if\s*\(typeof bumpWorkspaceTreeGen==='function'\)bumpWorkspaceTreeGen\(\);\s*await api\('\/api\/session\/update'/.test(panels)) throw new Error('workspace switch invalidation');
if (/S\.session\.workspace=path;\s*if\s*\(\s*typeof bumpWorkspaceTreeGen/.test(panels)) throw new Error('workspace switch double invalidation');

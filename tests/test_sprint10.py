...
def test_api_modules_exist(cleanup_test_sessions):
    ...
    base = pathlib.Path("/home/hermes/webui-mvp/api")
...
def test_server_py_under_700_lines(cleanup_test_sessions):
    """server.py should be under 700 lines after the split."""
    lines = len(pathlib.Path("/home/hermes/webui-mvp/server.py").read_text().splitlines())
    assert lines < 700, f"server.py is {lines} lines -- split may not have landed"

def test_api_config_has_cancel_flags(cleanup_test_sessions):
    src = pathlib.Path("/home/hermes/webui-mvp/api/config.py").read_text()

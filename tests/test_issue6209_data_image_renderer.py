...
const encodedSvg = 'data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3C%2Fsvg%3E';
...
console.log(JSON.stringify({
  ...
  streamEncodedSvg: _smdImgSrcAllowed(encodedSvg),
  settledRaster: _dataImageHtml(raster, 'chart') !== null,
  streamRaster: _smdImgSrcAllowed(raster),
}));
"""
...
@pytest.mark.skipif(NODE is None, reason="node not on PATH")
def test_url_encoded_svg_is_denied_in_both_renderers() -> None:
    cases = _node_contract_cases()
    assert cases["settledEncodedSvg"] is False
    assert cases["streamEncodedSvg"] is False


@pytest.mark.skipif(NODE is None, reason="node not on PATH")
def test_base64_raster_remains_supported_in_both_renderers() -> None:
    cases = _node_contract_cases()
    assert cases["settledRaster"] is True
    assert cases["streamRaster"] is True

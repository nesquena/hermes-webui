...
def wait_for_file_text(path: Path, expected: str, timeout: float = 3.0) -> str:
    deadline = time.time() + timeout
    last = ""
    while time.time() < deadline:
        if path.exists():
            ...
            if expected in last:
                return last
        time.sleep(0.05)
    raise AssertionError(f"{expected!r} was not written to {path}; last text: {last!r}")
...
def test_start_writes_pid_under_hermes_home_runs_foreground_no_browser_and_logs(tmp_path):
    ...
    try:
        ...
        fake_output = wait_for_file_text(fake_log, "bootstrap.py --no-browser --foreground")
...
def test_start_loads_dotenv_but_inline_overrides_win(tmp_path):
    ...
    try:
        fake_output = wait_for_file_text(fake_log, "fake-python args:")

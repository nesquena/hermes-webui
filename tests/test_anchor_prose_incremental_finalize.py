    ...
    parser_write(parser, text){
      ...
      if(options.sessionReferenceMarkup){
        ...
        const markup = parser.text.replace(
          'Answer: [abc123](session://abc123).',
          'Answer: <a class="session-link" href="/app/session/abc123">abc123</a>.');
...
def test_anchor_session_reference_rebuilds_from_full_authoritative_text():
    ...
    assert data["bodyText"] == "Answer: [abc123](session://abc123)."
    assert data["writtenText"] == "Answer: [abc123](session://abc123)."

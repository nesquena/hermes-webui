...
def test_deferred_hidden_stream_error_reattaches_or_restores_before_inline_error():
    ...
    assert "if(await _restoreSettledSession(source, {preserveVisibleOnShorterTerminalSnapshot:true})||_bailOutOfTerminalEventsFromStaleStream(source)) return;" in recovery_block
    assert recovery_block.find("if(await _restoreSettledSession(source, {preserveVisibleOnShorterTerminalSnapshot:true})||_bailOutOfTerminalEventsFromStaleStream(source)) return;") < recovery_block.rfind("_handleStreamError(source)")

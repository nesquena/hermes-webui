...
def test_sse_error_defers_while_page_hidden_until_tab_returns():
    ...
    assert error_handler.find("_deferStreamErrorIfPageHidden(source)") < error_handler.rfind(
        "_handleStreamError(source,retainedTransportGeneration)"
    )


def test_deferred_hidden_stream_error_reattaches_or_restores_before_inline_error():
    recovery_block = MESSAGES_JS.split(
        "function _reattachOrRestoreAfterDeferredStreamError(source,transportGeneration){", 1
    )[1].split("function _deferStreamErrorIfPageHidden(source)", 1)[0]
    ...
    assert "if(await _restoreSettledSession(source, {preserveVisibleOnShorterTerminalSnapshot:true,transportGeneration})) return;" in recovery_block
    assert recovery_block.find(
        "if(await _restoreSettledSession(source, {preserveVisibleOnShorterTerminalSnapshot:true,transportGeneration})) return;"
    ) < recovery_block.rfind("_handleStreamError(source,transportGeneration)")

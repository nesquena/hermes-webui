...
class TestRound6DefaultHonorsIsolationClamp:
    ...
    def test_get_last_workspace_uses_pinned_state_not_base(self, isolated_default_pinned, monkeypatch):
        env = isolated_default_pinned
        # The PINNED profile's own last_workspace selection...
        pinned_ws = env["tmp"] / "srv" / "pinned-ws"
        pinned_ws.mkdir(parents=True)
        lw = env["pinned"] / "webui_state" / "last_workspace.txt"
        lw.write_text(str(pinned_ws), encoding="utf-8")

        # ...and a POISONED global/base binding that must NOT win. Point the
        # global fallback at a file inside the BASE home to prove no base read.
        ...
        assert str(got) == str(pinned_ws.resolve()), (
            f"profile='default' in isolated mode must read the PINNED profile's "
            f"last_workspace.txt ({lw}), got {got!r}"
        )
    ...
    def test_new_session_binds_pinned_workspace(self, isolated_default_pinned):
        """Session creation with profile='default' lands on the pinned home."""
        import api.models as models

        env = isolated_default_pinned
        pinned_ws = env["tmp"] / "srv" / "pinned-session-ws"
        pinned_ws.mkdir(parents=True)
        lw = env["pinned"] / "webui_state" / "last_workspace.txt"
        lw.write_text(str(pinned_ws), encoding="utf-8")
        ...
        assert str(s.workspace) == str(pinned_ws.resolve()), (
            f"new_session(profile='default') in isolated mode must bind the "
            f"PINNED workspace, got {s.workspace!r}"
        )

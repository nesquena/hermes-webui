...
def _run(tmp_path, monkeypatch, result, **kw):
    ...
    return [event for event, _payload in events], payload, streaming.SESSIONS["tool_limit_session"]
...
def test_streaming_conformant_envelope_completes_the_turn(tmp_path, monkeypatch):
    kinds, payload, session = _run(tmp_path, monkeypatch, _envelope(_rewritten(), marked_idx=2, completed=True, final_response=NEW_ANSWER))

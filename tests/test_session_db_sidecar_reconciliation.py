...
def test_recovered_sidecar_derives_message_count_from_materialized_rows(tmp_path, monkeypatch):
    ...
    active_empty = models.Session.load(sid)
    assert active_empty is not None
    active_empty.messages = []
    active_empty.active_stream_id = "a" * 32
    active_empty.pending_user_message = "still typing"
    active_empty.save()
    assert len(json.loads(sidecar.read_text(encoding="utf-8"))["messages"]) == 5

    shrinking = models.Session.load(sid)
    assert shrinking is not None
    shrinking.messages = [{"role": "user", "content": f"replacement {i}"} for i in range(3)]
    shrinking.save()

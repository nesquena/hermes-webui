...
def test_theme_color_updates_all_meta_tags_not_only_the_id_tag():
    sync = _function_body(BOOT_JS, "function _syncThemeColorMeta")

    assert "document.querySelectorAll('meta[name=\"theme-color\"]')" in sync
    assert "meta.removeAttribute('media')" in sync
    assert "getPropertyValue('--bg')" in sync

    assert "document.querySelectorAll('meta[name=\"theme-color\"]')" in INDEX_HTML
    assert "removeAttribute('media')" in INDEX_HTML


def test_manifest_native_chrome_color_matches_dark_hermes_background():
    assert MANIFEST["theme_color"] == "#0D0D1A"
    assert MANIFEST["background_color"] == "#0D0D1A"

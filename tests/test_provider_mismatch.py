...
class TestFrontendModelProviderState:
    ...
    def test_new_session_carries_visible_picker_model_into_create_request(self):
        ...
        assert "window._activeProvider" in provider_assignment
        assert "S.session&&S.session.model_provider" in provider_assignment
        pos_explicit = provider_assignment.index("newModelState.model_provider")
        pos_active = provider_assignment.index("window._activeProvider")
        pos_prev = provider_assignment.index("S.session&&S.session.model_provider")

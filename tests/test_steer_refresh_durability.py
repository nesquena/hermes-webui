...
def test_frontend_consumes_steer_sse_and_settlement_keeps_control_rows():
    ...
    render_start = ui.index("function _anchorSceneSceneHasWorklogWorthyRows")
    render_end = ui.index("\nfunction ", render_start + 20)
    render_body = ui[render_start:render_end]
    assert "role==='control'" in render_body


def test_degraded_steer_warning_is_translated_in_non_english_locales():
    i18n = (ROOT / "static" / "i18n.js").read_text(encoding="utf-8")
    english = (
        "Steer was delivered, but it could not be saved and may disappear after refresh"
    )
    assert i18n.count("steer_delivery_not_durable:") >= 15
    assert i18n.count(f"steer_delivery_not_durable: '{english}'") == 1

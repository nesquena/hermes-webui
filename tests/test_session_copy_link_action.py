...
def test_conversation_filter_keeps_content_search_results_when_query_is_session_id():
    ...
    assert "const sidebarRows=_sessionRowsWithActiveEphemeralSession(_allSessions,runtimeContext);" in SESSIONS_JS
    assert "const searchMatches=_sessionSearchMergeMatches(sidebarRows,searchQueryRaw,_contentSearchResults);" in SESSIONS_JS
    assert "const allMatched=_ensureActiveSessionRowPresent(searchMatches,sidebarRows,runtimeContext);" in SESSIONS_JS

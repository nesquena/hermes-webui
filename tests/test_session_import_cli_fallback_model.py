...
def test_session_import_cli_refresh_matches_messages_despite_timestamp_type_differences(monkeypatch):
    ...
    monkeypatch.setattr(routes, "get_cli_sessions", lambda all_profiles=False: [{"session_id": session_id, "source_tag": "weixin", "raw_source": "weixin", "session_source": "messaging", "source_label": "WeChat"}])
...
def test_session_import_cli_refresh_rejects_prefix_if_non_timing_content_diverges(monkeypatch):
    ...
    monkeypatch.setattr(routes, "j", lambda _handler, payload, status=200, extra_headers=None: payload)
    monkeypatch.setattr(routes, "get_cli_session_messages", lambda sid, profile=None: fresh if sid == session_id else [])
    monkeypatch.setattr(routes, "get_cli_sessions", lambda all_profiles=False: [{"session_id": session_id, "source_tag": "telegram", "raw_source": "telegram", "session_source": "messaging", "source_label": "Telegram"}])
...
def test_sessions_endpoint_suppresses_duplicate_webui_state_projection(monkeypatch):
    ...
    monkeypatch.setattr(routes, "get_cli_sessions", lambda all_profiles=False: [duplicate_webui_projection, external_projection])

...
def test_frontend_subscribes_to_process_complete_event():
    ...
    # Option Z PIVOT: the browser must NOT re-POST the wakeup to /api/chat/start
    # anymore — the wakeup is started server-side by the drain thread. The
    # shared handler keeps the wakeup_prompt reference only for its empty-guard
    # + dedupe; the re-POST fetch was removed and is documented as a pivot.
    assert "wakeup_prompt" in js
    handler_start = js.index("function _handleProcessCompleteEvent")
    handler_end = js.index("// ── Clarify polling ──", handler_start)
    handler_src = js[handler_start:handler_end]
    assert "api/chat/start" not in handler_src, (
        "Option Z: _handleProcessCompleteEvent must NOT re-POST /api/chat/start; "
        "the agent wakeup is started server-side by the drain thread"
    )
    assert "Option Z" in handler_src, (
        "the demoted handler should document the Option Z pivot"
    )

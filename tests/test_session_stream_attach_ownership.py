...
def _messages_owner_script() -> str:
    return "\n".join(
        [
            _extract_function(MESSAGES_JS, "_dispatchExtensionTurnLifecycle"),
...
class FakeEventSource {
  constructor(url, options) {
    if (global.__failEventSourceConstruction) throw new Error('constructor failed');
    this.url = String(url || '');
...
function resetState(sid, stream, generation) {
  global.__failEventSourceConstruction = false;
...
async function run() {
  ...
  resetState('C', 'constructor-failure', 6);
  gate = queueStatus('constructor-failure');
  global.__failEventSourceConstruction = true;
  claim = claimAttach({ sid: 'C', stream: 'constructor-failure', generation: 6, ownerToken: 'constructor-failure' });
  gate.resolve({ active: true });
  results.constructorFailure = await claim;
  results.constructorFailureReattach = INFLIGHT.C.reattach;
  results.constructorFailureOpenSources = global.__openSources.length;
  results.pendingAfterConstructorFailure = Object.keys(_PENDING_LIVE_ATTACHES).length;
  ...
  results.inactiveReattach = INFLIGHT.I.reattach;
...
def test_live_ownership_claim_and_restore_sequences_behave():
    ...
    assert (
        data["constructorFailure"] is False
        and data["constructorFailureReattach"] is True
        and data["constructorFailureOpenSources"] == 0
        and data["pendingAfterConstructorFailure"] == 0
    )
    assert (
        data["inactive"] is False
        and data["inactiveOpenSources"] == 0
        and data["inactiveReattach"] is True
        and data["pendingAfterInactive"] == 0
    )

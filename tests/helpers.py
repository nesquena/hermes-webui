...
def git_subcommand_args(cmd: Sequence[str]) -> list[str]:
    ...
    ``api.subprocess_utils.noninteractive_git_argv``, which prefixes the
    executable with ``-c`` credential overrides. A stub that dispatches on the
    subcommand would otherwise have to hard-code that prefix and would break
    every time it changes, so derive it from the production helper instead — and
    assert it is present, so a runner that stops hardening its argv fails here
    rather than silently matching a stub that no longer describes it.
    """

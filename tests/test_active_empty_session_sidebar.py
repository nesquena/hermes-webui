...
def test_new_session_switches_sidebar_back_to_webui_source():
    ...
    assert "_sessionSourceFilters.some(source=>source!=='webui')" in new_session
    assert "_sessionSourceFilters=['webui']" in new_session
    assert "_sessionSourceFilter='webui'" in new_session
    assert "hermes-session-source-filters-v2" in new_session

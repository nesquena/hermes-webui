...
def test_compression_exhausted_apperror_clears_reference_ui_and_labels_error():
    ...
    assert "const incomingSessionSid=d.session&&typeof d.session==='object' ? String(d.session.session_id||'') : '';" in block
    assert "const continuationSid=d.new_session_id||d.continuation_session_id||'';" in block
    assert "if(d.session&&typeof d.session==='object'&&_terminalSession)" in block
    assert "S.session=_terminalSession;" in block
...
def test_apperror_matches_only_current_or_continuation_session_for_background_errors():
    ...
    assert "const incomingSessionSid=d.session&&typeof d.session==='object' ? String(d.session.session_id||'') : '';" in block
    assert "const continuationSid=d.new_session_id||d.continuation_session_id||'';" in block
    assert "const eventMatchesCurrent=!!(currentSid&&(" in block
    assert "eventSid===currentSid||continuationSid===currentSid||incomingSessionSid===currentSid" in block

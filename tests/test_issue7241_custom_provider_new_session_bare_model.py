...
def test_ungrouped_option_leaks_routing_id(driver_path):
    """Contract guard: with NO owning provider group the catalog stamp derives
    nothing. Before #6895 extraction then fell back to the full routing id (the
    leak #7241 reports); since #6895 _modelStateForSelect() strips the
    @custom:<slug>: prefix itself, so even an unstamped ungrouped option yields
    the bare model. Stamping still runs for grouped rows (see the other tests)."""
    out = _run_driver(driver_path, [])
    assert out["withoutMeta"]["model"] == BARE_MODEL, (
        "an unstamped @custom:<slug>:<model> option must still extract the bare "
        "model (#6895) — the routing id must never leak into the session model"
    )

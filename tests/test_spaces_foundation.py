...
def test_space_tool_adapter_rejects_ambient_current_selectors_for_non_current_widget_event_metadata_only(
    ...
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "tool-ambient-event-lab", "name": "Tool Ambient Event Lab"})
    spaces.upsert_widget(created["space_id"], {"id": "research-card", "kind": "prompt", "title": "Research Card"})
...
def test_space_tool_adapter_admin_rollback_rejects_conflicting_event_aliases_before_side_effects(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "tool-admin-rollback-conflict", "name": "Original"})
    original_event_id = created["revision_event_id"]
    updated = spaces.update_space(created["space_id"], {"name": "Patched"})
    patched_event_id = updated["revision_event_id"]

    conflicting_payloads = (
        {
            "spaceId": created["space_id"],
            "event_id": original_event_id,
            "revisionEventId": patched_event_id,
        },
        {
            "spaceId": created["space_id"],
            "eventId": patched_event_id,
            "args": [created["space_id"], original_event_id],
        },
    )
    ...
    detail = spaces.read_space_detail(created["space_id"])
    serialized = json.dumps(detail).lower()
    assert detail["name"] == "Patched"
    assert detail["revision_event_id"] == patched_event_id
    assert "renderer" not in serialized
    assert "api_key" not in serialized
    assert "secret_value_do_not_leak" not in serialized
    assert "<script" not in serialized
...
def test_space_tool_adapter_widget_restore_rejects_conflicting_event_positional_alias_before_side_effects(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "tool-widget-restore-conflict", "name": "Widget Restore Conflict"})
    original = spaces.upsert_widget(created["space_id"], {"id": "panel", "kind": "markdown", "title": "Original panel"})
    patched = spaces.patch_widget(created["space_id"], "panel", {"title": "Patched panel"})
    ...
    with pytest.raises(ValueError, match="selector aliases"):
        spaces.run_space_tool(
            ...
            {
                "spaceId": created["space_id"],
                "eventId": patched["revision_event_id"],
                "widgetId": "panel",
    ...
    widget = spaces.read_widget(created["space_id"], "panel")
    serialized = json.dumps(widget).lower()
    assert widget["title"] == "Patched panel"
    assert "renderer" not in serialized
    assert "source" not in serialized
    assert "secret_value_do_not_leak" not in serialized
    assert "<script" not in serialized
...
def test_camera_stream_tool_returns_output_compaction_receipt_metadata_only(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    installed = spaces.install_template("camera")
    space_id = installed["space"]["space_id"]
    ...
    result = spaces.run_space_tool(
        ...
        {
            "space_id": space_id,
            "title": "Front gate <script>ignored</script>",
            "url": "http://10.0.0.8:554/live?token=SECRET_VALUE_DO_NOT_LEAK",
            "approved": True,
            "approval_id": "approval-456",
            "renderer": "<script>steal()</script>",
            "source": "raw camera source SECRET_VALUE_DO_NOT_LEAK",
            "api_auth": "bearer SECRET_VALUE_DO_NOT_LEAK",
        },
    )
...
def test_install_model_setup_template_creates_safe_provider_setup_widgets(monkeypatch, tmp_path):
    ...
    assert installed["prompt_preflight"]["boundary"] == "model_provider_template"
    assert installed["prompt_preflight"]["status"] == "pass"
    assert installed["prompt_preflight"]["metadata_only"] is True
    assert installed["prompt_preflight"]["raw_prompt_stored"] is False
    assert installed["autonomy_policy"]["action"] == "space.template.install.model_provider"
    assert installed["autonomy_policy"]["approval_gates"] == ["destructive_external_action", "credential_change"]
    assert installed["autonomy_policy"]["prompt_preflight_status"] == "pass"
...
def test_model_setup_template_install_route_returns_safe_metadata(monkeypatch, tmp_path):
    ...
    assert len(body["installed_widgets"]) == 4
    assert body["prompt_preflight"]["boundary"] == "model_provider_template"
    assert body["prompt_preflight"]["status"] == "pass"
    assert body["prompt_preflight"]["metadata_only"] is True
    assert body["prompt_preflight"]["raw_prompt_stored"] is False
    assert body["autonomy_policy"]["action"] == "space.template.install.model_provider"
    assert body["autonomy_policy"]["approval_gates"] == ["destructive_external_action", "credential_change"]
    assert body["autonomy_policy"]["prompt_preflight_status"] == "pass"
    assert body["autonomy_policy"]["model_route_hint"] == "hint:local"
    assert body["autonomy_policy"]["metadata_only"] is True
    assert body["progress_event"]["event_type"] == "tool.completed"
    assert body["progress_event"]["family"] == "tool"
    assert body["progress_event"]["run_id"] == f"template.install:{body['space']['space_id']}"
...
def test_space_tool_adapter_model_setup_template_install_returns_policy_receipts(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    ...
    result = spaces.run_space_tool(
        "space.template.install",
        {
            ...
            "space_id": "tool-model-setup-demo",
            "renderer": "<script>steal()</script>",
            "source": "raw provider source should not leak",
            "api_key": "SECRET_VALUE_DO_NOT_LEAK",
            "token": "TOKEN_VALUE_DO_NOT_LEAK",
            "password": "PASSWORD_VALUE_DO_NOT_LEAK",
            "widgets": [{"id": "unsafe", "html": "<img src=x onerror=steal()>"}],
    ...
    assert result["ok"] is True
    assert result["action"] == "space.template.install"
    assert result["template"] == "model-setup"
    assert result["space"]["space_id"] == "tool-model-setup-demo"
    assert result["space"]["name"] == "Model Provider Setup"
    assert [widget["id"] for widget in result["installed_widgets"]] == [
        "model-provider-status",
        "model-local-runtime",
        "model-settings-review",
    ...
    assert result["prompt_preflight"]["boundary"] == "model_provider_template"
    assert result["prompt_preflight"]["status"] == "pass"
    assert result["prompt_preflight"]["metadata_only"] is True
    assert result["prompt_preflight"]["raw_prompt_stored"] is False
    assert result["autonomy_policy"]["action"] == "space.template.install.model_provider"
    assert result["autonomy_policy"]["approval_gates"] == ["destructive_external_action", "credential_change"]
    assert result["autonomy_policy"]["prompt_preflight_status"] == "pass"
    assert result["autonomy_policy"]["model_route_hint"] == "hint:local"
    assert result["autonomy_policy"]["metadata_only"] is True
    assert result["progress_event"]["event_type"] == "tool.completed"
    assert result["progress_event"]["family"] == "tool"
...
def test_activate_space_route_accepts_camelcase_session_id_metadata_only(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "activate-session-camel", "name": "Activate Session Camel"})
    ...
    monkeypatch.setattr(models, "SESSION_DIR", config.SESSION_DIR)
    models.SESSIONS.clear()
    session = models.Session(session_id="session_activate_session_camel", workspace=str(tmp_path))
    session.messages = [{"role": "user", "content": "SECRET_MESSAGE_DO_NOT_LEAK renderer <script>"}]
    session.save(skip_index=True)
    ...
    handled, status, body = _route_post(
        ...
        {
            "spaceId": created["space_id"],
            "sessionId": "session_activate_session_camel",
            "renderer": "<script>activateSessionLeak()</script>",
            "api_key": "SECRET_VALUE_DO_NOT_LEAK",
        },
    )
    ...
    activated_session = models.get_session("session_activate_session_camel")
    assert activated_session is not None
    assert activated_session.active_space_id == created["space_id"]
    serialized = json.dumps(body).lower()
    assert "activatesessionleak" not in serialized
    assert "<script" not in serialized
    assert "renderer" not in serialized
    assert "api_key" not in serialized
    assert "secret_message_do_not_leak" not in serialized
    assert "secret_value_do_not_leak" not in serialized


def test_activate_space_route_rejects_conflicting_session_aliases_before_mutation(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "activate-session-conflict", "name": "Activate Session Conflict"})
    ...
    monkeypatch.setattr(models, "SESSION_DIR", config.SESSION_DIR)
    models.SESSIONS.clear()
    first = models.Session(session_id="session_activate_first", workspace=str(tmp_path))
    first.save(skip_index=True)
    second = models.Session(session_id="session_activate_second", workspace=str(tmp_path))
    ...
    handled, status, body = _route_post(
        ...
        {
            "space_id": created["space_id"],
            "session_id": "session_activate_first",
            "sessionId": "session_activate_second",
            "renderer": "<script>sessionConflictLeak()</script>",
            "api_key": "SECRET_VALUE_DO_NOT_LEAK",
        },
    )
    ...
    assert first_session.active_space_id is None
    assert second_session.active_space_id is None
    serialized = json.dumps(body).lower()
    assert "sessionconflictleak" not in serialized
    assert "<script" not in serialized
    assert "renderer" not in serialized
    assert "api_key" not in serialized
    assert "secret_value_do_not_leak" not in serialized
...
def test_deactivate_space_route_accepts_camelcase_session_id_metadata_only(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "deactivate-session-camel", "name": "Deactivate Session Camel"})
    ...
    monkeypatch.setattr(models, "SESSION_DIR", config.SESSION_DIR)
    models.SESSIONS.clear()
    session = models.Session(session_id="session_deactivate_camel", workspace=str(tmp_path))
    session.active_space_id = created["space_id"]
    session.messages = [{"role": "user", "content": "SECRET_DEACTIVATE_MESSAGE_DO_NOT_LEAK renderer <script>"}]
    session.save(skip_index=True)

    handled, status, body = _route_post(
        "/api/spaces/deactivate",
        {
            "sessionId": "session_deactivate_camel",
            "renderer": "<script>deactivateLeak()</script>",
            "api_key": "SECRET_VALUE_DO_NOT_LEAK",
        },
    )
    ...
    assert "messages" not in body["session"]
    deactivated_session = models.get_session("session_deactivate_camel")
    assert deactivated_session is not None
    assert deactivated_session.active_space_id is None
    serialized = json.dumps(body).lower()
    assert "deactivateleak" not in serialized
    assert "<script" not in serialized
    assert "renderer" not in serialized
    assert "api_key" not in serialized
    assert "secret_deactivate_message_do_not_leak" not in serialized
    assert "secret_value_do_not_leak" not in serialized


def test_deactivate_space_route_rejects_conflicting_session_aliases_before_mutation(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "deactivate-session-conflict", "name": "Deactivate Session Conflict"})
    ...
    monkeypatch.setattr(models, "SESSION_DIR", config.SESSION_DIR)
    models.SESSIONS.clear()
    first = models.Session(session_id="session_deactivate_first", workspace=str(tmp_path))
    first.active_space_id = created["space_id"]
    first.save(skip_index=True)
    second = models.Session(session_id="session_deactivate_second", workspace=str(tmp_path))
    second.active_space_id = created["space_id"]
    second.save(skip_index=True)

    handled, status, body = _route_post(
        "/api/spaces/deactivate",
        {
            "session_id": "session_deactivate_first",
            "sessionId": "session_deactivate_second",
            "renderer": "<script>deactivateConflictLeak()</script>",
            "api_key": "SECRET_VALUE_DO_NOT_LEAK",
        },
    )
    ...
    assert first_session is not None
    assert second_session is not None
    assert first_session.active_space_id == created["space_id"]
...
def test_widget_event_rejects_nested_conflicting_runtime_message_aliases_metadata_only(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "nested-runtime-conflict", "name": "Nested Runtime Conflict"})
    spaces.upsert_widget(created["space_id"], {"id": "sandbox", "kind": "html", "title": "Sandbox"})

    for payload in (
        {"message": {"type": "capy:ready", "messageType": "capy:agent:prompt"}, "query": "safe"},
        {"event": {"message_type": "capy:resize", "messageType": "capy:agent:prompt"}, "query": "safe"},
        {"messages": [{"type": "capy:ready", "message_type": "capy:resize"}], "query": "safe"},
    ):
        with pytest.raises(ValueError, match="runtime contract"):
            spaces.queue_widget_event(
                created["space_id"],
    ...
    serialized = json.dumps(events).lower()
    assert events == []
    assert "capy:ready" not in serialized
    assert "capy:resize" not in serialized
...
def test_widget_event_route_rejects_ambient_current_selectors_before_queue_metadata_only(monkeypatch, tmp_path):
    spaces = _load_spaces(monkeypatch, tmp_path, enabled=True)
    created = spaces.create_space({"space_id": "ambient-route-runtime", "name": "Ambient Route Runtime"})
    spaces.upsert_widget(created["space_id"], {"id": "sandbox", "kind": "html", "title": "Sandbox"})
    ...
    handled, status, body = _route_post(
        "/api/spaces/widget/event",
        {
            ...
            "prompt": "Use SECRET_VALUE_DO_NOT_LEAK <script>bad()</script>",
            "payload": {
                "query": "safe ambient route prompt",
                "renderer": "<script>bad()</script>",
                "apiKey": "SECRET_VALUE_DO_NOT_LEAK",
                "source": "SECRET_SOURCE",
            },

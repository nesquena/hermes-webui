...
def test_render_uses_single_pass_partition_helper():
    ...
    assert "_renderSidebarRowsFromRawSessions(sessionsRaw, [...referenceRaw, ..._scopedSidebarReferenceRows(isCliView, projectIdFor)], searchActive)" in render_body
    ...
    assert "? _renderSidebarRowsFromRawSessions(webuiSessionsRaw, [...webuiReferenceRaw, ..._scopedSidebarReferenceRows(false, projectIdFor)], searchActive).length" in render_body
    assert "? _renderSidebarRowsFromRawSessions(cliSessionsRaw, [...cliReferenceRaw, ..._scopedSidebarReferenceRows(true, projectIdFor)], searchActive).length" in render_body
...
def test_partition_helper_keeps_raw_source_counts_while_render_owns_visible_counts():
    ...
    assert "function _renderSidebarRowsFromRawSessions(sessionsRaw, referenceSessionsRaw, searchActive=false){" in SESSIONS_JS
    assert "_attachChildSessionsToSidebarRows(_collapseSessionLineageForSidebar(sessionsRaw), sessionsRaw, referenceRows, searchActive)" in SESSIONS_JS

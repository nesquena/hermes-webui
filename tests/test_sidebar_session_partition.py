...
def test_render_uses_single_pass_partition_helper():
    ...
    assert "const lineageIndex=_buildSidebarLineageIndex(" in render_body
    assert "_partitionSidebarSessionRows(allMatched, activeSidForSidebar, lineageIndex)" in render_body
    assert "const lineageScope={" in render_body
    assert "_renderSidebarRowsFromRawSessions(" in render_body
    assert "sessionsRaw, [...referenceRaw, ..._scopedSidebarReferenceRows(isCliView, lineageIndex)], lineageScope, lineageIndex)" in render_body
    ...
    assert "{...lineageScope, isCli:false}, lineageIndex).length" in render_body
    assert "cliSessionsRaw, [...cliReferenceRaw, ..._scopedSidebarReferenceRows(true, lineageIndex)]," in render_body
    assert "{...lineageScope, isCli:true}, lineageIndex).length" in render_body
...
def test_partition_helper_keeps_raw_source_counts_while_render_owns_visible_counts():
    ...
    assert "function _renderSidebarRowsFromRawSessions(sessionsRaw, referenceSessionsRaw, lineageScope, lineageIndex){" in SESSIONS_JS
    ...
    assert "_collapseSessionLineageForSidebar(sessionsRaw,index), sessionsRaw, referenceRows, durableLineageIdsByScope, index)" in SESSIONS_JS

...
def test_fallback_list_contains_free_tier_entries():
    ...
    contributor's six free-tier entries so offline / test envs see them."""
    or_entries = [m for m in config._FALLBACK_MODELS if m.get("provider") == "OpenRouter"]
    assert len(or_entries) >= 6, "fallback list should include at least the 6 free-tier entries"
    free_labels = [m["label"] for m in or_entries if "free" in m["label"].lower()]
    assert len(free_labels) >= 6, f"expected ≥6 free-tier entries in fallback, got {len(free_labels)}"
...
def test_openrouter_group_uses_live_fetch_when_available(monkeypatch):
    ...
    def _fake_urlopen(req, timeout=None):
        captured_calls.append(req.full_url if hasattr(req, 'full_url') else str(req))
        return _FakeResponse(fake_payload)

    monkeypatch.setattr(urllib.request, "urlopen", _fake_urlopen)
    # Also reset the hermes_cli cache so its fetch path runs fresh
    try:
        from hermes_cli import models as _hm
        monkeypatch.setattr(_hm, "_openrouter_catalog_cache", None, raising=False)
    except Exception:
...
def test_openrouter_falls_back_to_static_when_live_fails(monkeypatch):
    ...
    # The 6 hardcoded free-tier entries MUST be in the fallback
    fallback_ids = {m["id"] for m in or_group["models"]}
    assert "minimax/minimax-m2.5:free" in fallback_ids, \
        "static fallback must include the contributor's hardcoded free-tier entries"
...
def test_free_tier_cap_prevents_picker_drowning(monkeypatch):
    ...
    # Generate 50 free-tier items to exceed the cap
    items = []
    for i in range(50):
        items.append({
    ...
    # Reset hermes_cli curated cache so its fetch attempts the same fake
    try:
        from hermes_cli import models as _hm
        monkeypatch.setattr(_hm, "_openrouter_catalog_cache", None, raising=False)
    except Exception:
    ...
    # The cap is 30 — fewer is fine if hermes_cli also returned :free variants
    # (those wouldn't be deduped here since seen_ids tracks both surfaces)
    assert len(free_added_ids) <= 50, "should not exceed the items provided"
    # And we should see *some* additions, not zero
...
def test_openrouter_dedupe_curated_and_free_tier(monkeypatch):
    ...
    # Make hermes_cli return the same id (free, but already curated as tool-supporting)
    import sys
    fake_module = type(sys)("hermes_cli.models")

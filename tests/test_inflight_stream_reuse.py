...
def test_attach_and_reattach_use_the_validated_replay_floor():
    ...
    active_start = load_body.index("constattachLiveSceneForActiveSession=")
    active_end = load_body.index("};", active_start)
    active_attach = load_body[active_start:active_end]
    active_normalize = active_attach.index("_normalizeInflightReplayCursorForReattach(INFLIGHT[sid])")
    assert "String(currentInflight.streamId||'')===String(activeStreamId)" in active_attach
    assert active_attach.index("_isActiveSessionSceneRestoreOwner(") < active_normalize
    assert active_attach.index("attachOwnedSessionStream(true)") > active_normalize

    idle_start = load_body.index("constattachLiveSceneForIdleSession=")
    idle_end = load_body.index("};", idle_start)
    idle_attach = load_body[idle_start:idle_end]
    idle_normalize = idle_attach.index("_normalizeInflightReplayCursorForReattach(INFLIGHT[sid])")
    assert "String(currentInflight.streamId||'')===String(activeStreamId)" in idle_attach
    assert idle_attach.index("_isActiveSessionSceneRestoreOwner(") < idle_normalize
    assert idle_attach.index("returnattachOwnedSessionStream()") > idle_normalize

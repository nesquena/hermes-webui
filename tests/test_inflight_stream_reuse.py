...
def test_turn_admission_requires_exact_owner_and_releases_once(tmp_path, monkeypatch):
    ...
    # A cancelling owner must not take the exact-admission self-bypass.  Even an
    # old timestamp and absent STREAMS transport cannot retire a tokenized row;
    # the admission lifecycle remains the authority for its release.
    with config.ACTIVE_RUNS_LOCK:
        config.ACTIVE_RUNS[admission.stream_id]["phase"] = "cancelling"
        config.ACTIVE_RUNS[admission.stream_id]["cancelled_at"] = 1.0
    assert routes._active_run_stream_for_session(
        session.session_id,
        admission=admission,
    ) == admission.stream_id
    with config.ACTIVE_RUNS_LOCK:
        config.ACTIVE_RUNS[admission.stream_id]["phase"] = "worker"
        config.ACTIVE_RUNS[admission.stream_id].pop("cancelled_at", None)

...
def _move_sidecar_field_after_messages(path, field):
    """Rewrite one valid top-level field immediately after ``messages``."""
    import json
    ...
    serialized = json.dumps(reordered, ensure_ascii=False, indent=2)
    assert serialized.index('"messages"') < serialized.index(f'"{field}"')
    path.write_text(serialized, encoding="utf-8")


def _get_paginated_session_payload(routes, Session, child, monkeypatch):
    """Drive the real 30-row ``GET /api/session`` route in the isolated store."""
    import json
    ...
    return json.loads(handler.wfile.getvalue().decode("utf-8"))["session"]
...
def test_paginated_session_route_keeps_history_above_zero_watermark_snapshot(
    ...
    payload = _get_paginated_session_payload(
        routes, Session, child, monkeypatch
    )
...
@pytest.mark.parametrize(
    ("sidecar_id", "late_field"),
    [
        ("anc_0", "pre_compression_snapshot"),
        ("anc_1", "parent_session_id"),
    ],
    ids=["late-snapshot-flag", "late-parent-id"],
)
def test_paginated_route_fails_closed_when_lineage_proof_field_follows_messages(
    deep_lineage, monkeypatch, sidecar_id, late_field
):
    """A prefix stub may not turn an absent proof field into a constructor default.

    The complete sidecar is valid, but moving either proof-bearing field after
    ``messages`` makes it unavailable to ``load_metadata_only()``. The shortcut
    must decline and the real route must retain the older contributing rows.
    """
    routes, Session, child = deep_lineage(
        hops=2, sentinel={1}, ancestor_rows=4, child_rows=2
    )
    _move_sidecar_field_after_messages(
        routes.SESSION_DIR / f"{sidecar_id}.json", late_field
    )
    loaded = _counting_load(routes, Session, monkeypatch)

    payload = _get_paginated_session_payload(routes, Session, child, monkeypatch)
    ...
    assert [m["role"] for m in payload["messages"]] == ["user", "assistant"] * 5
    assert payload["_messages_offset"] == 0
    assert payload["_messages_truncated"] is False
    assert payload["message_count"] == 10
    assert [sid for sid in loaded if sid.startswith("anc_")] == ["anc_1", "anc_0"]


def test_paginated_route_keeps_fast_path_with_complete_prefix_proof(
    deep_lineage, monkeypatch
):
    """Positive control: complete all-sentinel prefixes still avoid full loads."""
    routes, Session, child = deep_lineage(
        hops=2, sentinel=True, ancestor_rows=4, child_rows=2
    )
    loaded = _counting_load(routes, Session, monkeypatch)

    payload = _get_paginated_session_payload(routes, Session, child, monkeypatch)

    assert _contents(payload["messages"]) == ["child-0", "child-1"]
    assert [sid for sid in loaded if sid.startswith("anc_")] == []

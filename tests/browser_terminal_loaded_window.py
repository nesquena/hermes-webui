                ...
                for engine in os.environ.get('BROWSERS', 'chromium,webkit').split(','):
                    ...
                    try:
                        for event, mode, fade, words, width in [(event, mode, fade, words, width)
                            ...
                            result = page.evaluate(r"""async ([words,event])=>{
                              ...
                              source.emit(event,{session_id:'fixture',type:event==='cancel'?'cancelled':'rate_limit',message:'Synthetic terminal',session:{session_id:'fixture',messages:[...history,{role:'user',content:'Inspect fixture'},{role:'assistant',content:answer}],message_count:3302,tool_calls:[]}},'run-fixture:1002');
                            ...
                            print(json.dumps(dict(event=event,engine=engine,mode=mode,fade=fade,words=words,width=width,**result)),flush=True)
                            session.clear()

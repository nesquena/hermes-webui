...
- Skills panel detail views now render the selected `SKILL.md` content again. The WebUI no longer passes profile-resolved absolute skill paths into Hermes Agent `skill_view()`, which rejects non-relative patterns on newer Agent versions and previously left the detail pane blank.

...
- **Option Z live-view + SSE backpressure fixes** (Isla Liu, fork prototype; two tightly-coupled follow-ups to Option Z, on top of `481ddb9`). **Defect B — server-initiated wakeup turn was not shown live (needed manual refresh):** Option Z starts the wakeup turn server-side via `start_session_turn → _start_chat_stream_for_session`, which only emits the turn's token/tool/stream_end frames into the per-turn `STREAMS[stream_id]`. The browser only opens an `EventSource(/api/chat/stream?…)` when IT POSTs `/api/chat/start`, so a server-initiated turn had a STREAMS channel no EventSource was attached to; the per-session `/api/session/stream` only carried `process_complete` (now a state no-op post-pivot), so an already-open tab rendered nothing until a manual refresh re-read persisted state. Fix: `api/routes.py::start_session_turn` now, after a successful start, fans a lightweight `server_turn_started {session_id, stream_id, source}` frame onto the existing per-session live-view channel via the **non-creating** `background_process.get_session_channel()` (closed-tab path stays a pure no-op; Option Z headline unaffected). `static/messages.js::startSessionStream` handles it by attaching the **existing** chat-stream renderer (`attachLiveStream` — the exact path `/api/chat/start` uses; no second renderer hand-rolled), with idempotency guards (`S.activeStreamId`/`LIVE_STREAMS[sid]` already on that stream → bail) so a concurrent per-turn stream never double-renders. **Defect A — SSE thread exhaustion with multiple tabs:** `server.py` `QuietHTTPServer(ThreadingHTTPServer)` is one OS thread per connection, no pool cap (`request_queue_size=64`); a slow/backgrounded tab whose TCP recv window fills makes `handler.wfile.write()/flush()` block indefinitely, pinning the worker thread for the connection's whole lifetime (it never reaches `finally: unsubscribe`, so the SessionChannel reaper can't reclaim it either) → threads pile up → "streaming pending". Fix: new `api/streaming.py::SSE_WRITE_DEADLINE_SECONDS = 20.0` + `_sse_set_write_deadline(handler)` arms a socket-level write timeout once after `end_headers()`, applied uniformly to all 6 long-lived SSE endpoints (chat-stream, terminal, gateway, approval, clarify, session). A healthy write completes in << 1 ms so the deadline never trips for a live tab; a backpressured socket raises `socket.timeout` — which IS `TimeoutError` on Python 3.10+ and is already in `routes._CLIENT_DISCONNECT_ERRORS` — so each handler's existing `except _CLIENT_DISCONNECT_ERRORS:` breaks the loop, `finally` drops the subscriber, the thread is released, the browser's EventSource auto-reconnects (SessionChannel already buffers offline), and the 60s-grace / 4h-idle reaper can finally collect the channel. Least-invasive by design: zero new threads/deps, no ThreadPoolExecutor server swap (explicitly gated behind evidence and avoided). Closed-tab wakeup (the Option Z headline) is strictly unregressed — both changes are additive. Tests: new `tests/test_optionz_liveview_perf.py` (9, incl. `test_server_turn_streams_to_session_channel` for Defect B and `test_sse_write_timeout_drops_slow_subscriber` for Defect A); full suite 5677 passed / 0 regressions vs the 5679 baseline; headline `test_server_side_wakeup_when_idle_no_tab` stays green.

- **Option Z PIVOT — server-side `process_complete` wakeup is now PRIMARY; browser SSE demoted to live-view only** (Isla Liu, fork prototype; supersedes the browser-mediated wakeup of Option B / PR #2279 / Option X for the wakeup trigger itself). The entire prior `process_complete` wakeup line of work was browser-mediated: the drain thread emitted an SSE frame and an open tab re-POSTed `wakeup_prompt` to `/api/chat/start`. That is structurally wrong for the primary background-task use case — "fire a long background task, **close the tab**, come back later" — because a closed tab can never re-POST, so the agent never woke. Option Z fixes this by starting the agent turn **directly server-side, with no browser round-trip**, exactly how the CLI / Telegram / Discord / gateway hosts already self-wake from a `notify_on_complete` completion. New reusable headless entrypoint `api/routes.py::start_session_turn(session_id, message, *, source="process_wakeup")` extracts the minimal core that `/api/chat/start` already reaches via `_handle_chat_start` → `_start_chat_stream_for_session` (same session-load → workspace → model/provider resolution → daemon-worker-thread turn; **no second agent-construction path**). `api/background_process.py::_process_one` now, AFTER the existing SSE emit + `PENDING_PROCESS_COMPLETIONS` marker (both kept for live-view), branches: if a turn is active (`ACTIVE_RUNS` has a row for the session via the new `_session_has_active_turn`) it does NOT start a turn and leaves the marker for PR #2279's next-turn drain; if idle it calls `_start_server_side_wakeup_turn`, which spawns a short-lived daemon thread (the drain loop never blocks) that invokes `start_session_turn` with the `[IMPORTANT: Background process … completed …]` prompt from `format_wakeup_prompt`. Concurrency/idempotency reuse the existing guards unchanged: `PROCESS_COMPLETE_EVENTS_SEEN` + registry `_completion_consumed` dedupe a process to one wakeup; a human `/api/chat/start` racing the wakeup is serialized by `_start_chat_stream_for_session`'s per-session agent lock + active-stream guard (loser gets `_status=409`, marker retained so the next-turn drain still delivers it). Browser change: `static/messages.js::_handleProcessCompleteEvent` no longer re-POSTs `/api/chat/start` — the toast, the diagnostic `/api/process-complete-ack`, the `_seenProcessCompleteIds` dedupe, and the `S.busy` short-circuit are kept (harmless no-ops for wakeup). The Option X `/api/session/stream` + `SessionChannel` + reaper machinery is **kept intact**, demoted in role only: an open tab streams the server-initiated turn live; a closed tab gets the persisted result on next session load — exactly like CLI behavior. The H1/H2/H3 hypotheses from the superseded card (S.busy race-gate, startSessionStream, browser re-POST) are N/A because the wakeup trigger is no longer in the browser. **Closed-tab wakeup now works = parity with CLI / Telegram / gateway.** Tests: 4 new in `tests/test_session_channel_option_x.py` (`test_server_side_wakeup_when_idle_no_tab` is the headline closed-tab proof; plus active-turn defer, dedupe, live-view), and `test_process_complete_wakeup.py` updated to assert the pivot (handler no longer re-POSTs chat-start).

- **Persistent per-session SSE channel — `/api/session/stream` (Option X architecture fix; supersedes #2242 + #2279 wakeup gap)** (Isla Liu, fork prototype). The earlier `process_complete` SSE event (Option B drain) and the absorbed PR #2279 (Option A next-turn drain) both bridge the `terminal(notify_on_complete=true)` agent-wakeup gap, but neither covers the case where the background process exits BETWEEN agent turns: at that point `STREAMS` has been torn down by `/api/chat/stream`'s `finally`, so the drain thread has no live SSE channel to push onto, and Option A only runs at the next user-initiated turn. Option X adds a long-lived SSE channel keyed on WebUI `session_id` (not `stream_id`) that survives across turns. New module surface in `api/background_process.py`: `SessionChannel` class with refcounted `subscribe`/`unsubscribe`/`emit` and a `SESSION_CHANNELS: dict[str, SessionChannel]` registry guarded by `SESSION_CHANNELS_LOCK`. A daemon reaper thread (`start_session_channel_reaper`, started in `server.py` alongside the existing process_complete drain) runs every 60s and collects channels whose subscribers have been empty for `SESSION_CHANNEL_SUBSCRIBER_GRACE_SECS` (60s default) or whose `created_at` is older than `SESSION_CHANNEL_IDLE_TTL_SECS` (4h default, configurable in `api/config.py`); a channel with at least one live subscriber is never collected. New SSE endpoint `GET /api/session/stream?session_id=<sid>` in `api/routes.py` (`_handle_session_sse_stream`) mirrors the approval/clarify framing — same `text/event-stream` + `X-Accel-Buffering: no` + 5s keepalive comment, same `_CLIENT_DISCONNECT_ERRORS` catch in `finally`, with subscribe/unsubscribe driven by the `SessionChannel` instead of the approval `_lock` table. `_emit_to_session_streams` in `api/background_process.py` is now a dual-emit: in addition to the existing best-effort `STREAMS` push (kept as defense-in-depth so an in-turn `process_complete` still reaches the live chat-stream), it also pushes onto `SESSION_CHANNELS[session_id]` if one exists. Frontend (`static/messages.js` + `static/sessions.js`) gains `startSessionStream`/`stopSessionStream` wired into `loadSession` / `newSession` (open on session mount, close on switch) and routes `process_complete` events through `_handleProcessCompleteEvent`, a shared handler that the in-turn `STREAMS` EventSource also calls; module-scope `_seenProcessCompleteIds` dedupes across both delivery paths so dual-emit never causes a double wakeup. Race gate added per agents-manager §3 design: when `S.busy` is true the handler short-circuits — Option A's next-turn drain will replay the missed event on the next idle moment, and re-POSTing on a live turn would race `ACTIVE_RUNS` and get a 409 anyway. EventSource auto-closes on tab close (server detects disconnect, unsubscribes); `readyState=CLOSED` triggers a 5s one-shot reconnect. 18-test regression suite (`tests/test_session_channel_option_x.py`) covers the `SessionChannel` subscribe/emit/unsubscribe lifecycle, slow-subscriber drop semantics, reaper grace period, reaper idle-TTL cap, reaper-keeps-live-subscriber, registry-level reaper integration, dual-emit when STREAMS is empty, dual-emit when STREAMS is busy, and source-grep wiring checks for the route, server-boot, and frontend.

- **`process_complete` SSE event — agent wakeup on `terminal(notify_on_complete=true)`** (Isla Liu, fork prototype; Option B layer for #2278; coordinates with PR #2279 which ships Option A). Before this change, `terminal(background=true, notify_on_complete=true)` produced no wakeup in WebUI sessions: the background process exited, the agent's `tools.process_registry.completion_queue` queued the completion event, and nothing read from it — the agent only resumed when the user typed something. CLI and gateway adapters (Telegram, Discord, etc.) already drained this queue inside their main loops; WebUI did not. This PR closes the gap with two complementary layers: **(B, proactive)** new `api/background_process.py` drain thread spawned at server startup pulls from `completion_queue`, formats the same `[IMPORTANT: …]` synthetic message CLI/gateway produce, and emits a `process_complete` SSE event onto the matching session's stream so the user sees a toast and the frontend re-POSTs `wakeup_prompt` to `/api/chat/start`. **(A, next-turn drain)** absorbed from PR #2279 by @franksong2702 (attribution preserved in commit history): `api/streaming.py` adds `_drain_webui_process_notifications(session_id)` that drains any remaining queue entries at the start of the next agent turn and prepends the formatted notification onto the user message, providing a safety net when the SSE stream was disconnected at the moment of completion. Both paths share a single dedupe set (`PROCESS_COMPLETE_EVENTS_SEEN`) so a process can only wake the agent once. Env wiring: `HERMES_SESSION_PLATFORM=webui`, `HERMES_SESSION_ID=<sid>`, `HERMES_SESSION_CHAT_ID=<sid>`, and `HERMES_SESSION_KEY=<sid>` are exported into both the thread-local agent env and the process env — `tools/terminal_tool.py:~1940` gates `pending_watchers.append` on `HERMES_SESSION_PLATFORM`, so without this export `notify_on_complete` events never enqueued. `api/routes.py` atomically discards `PENDING_PROCESS_COMPLETIONS` for the session in `_start_chat_stream_for_session`, mirroring the goal_continue marker pattern (#1932). `static/messages.js` subscribes to the new event, shows a small toast, and re-POSTs `wakeup_prompt`. `api/process-complete-ack` endpoint provides a stable diagnostic hook for the frontend. Backward compatible: old WebUI builds graceful-ignore unknown SSE events; clients that never receive the event simply never re-POST and the marker drains harmlessly on the next normal `/api/chat/start`. No DB schema change, no new dependency.

### Fixed

- **Cross-session `notify_on_complete` wakeup misroute — per-turn session identity raced on a process-global env slot** (Isla Liu, fork prototype; root-cause fix for the failure mode PR #2242's server-side wakeup introduced, RCA `t_f62ff1e8`). With 2+ concurrent WebUI sessions (different boards) each watching a `terminal(background=true, notify_on_complete=true)` task, a completed background task could wake the **wrong** session (observed: `agent.log:6632` — `watch.sh` spawned by session `47ec28f66dff` delivered its server-side wakeup turn to session `20260518_161627_60b5f4`). **Root cause (verified line-by-line, deterministically reproduced with the real `tools.approval` + `gateway.session_context` modules):** WebUI bound per-turn session identity ONLY to the process-global `os.environ['HERMES_SESSION_KEY']` (`api/streaming.py` turn-start) and released the env lock *before* the agent ran (`"Lock released — agent runs without holding it"`). WebUI never called any contextvar setter, so `gateway.session_context._SESSION_KEY` stayed `_UNSET` and `tools.approval.get_current_session_key` — the exact call a `notify_on_complete` background spawn makes in `tools/terminal_tool.py:~1928` — fell back to that racy process-global slot. Two concurrent turns therefore raced on one slot: session A's spawn captured the session_key the *most-recently-started* turn wrote, that contaminated value was pinned into `ProcessSession.session_key`, and at completion `api/background_process.py` resolved it through `PROCESS_SESSION_INDEX` and started the server-side wakeup turn for the wrong session. The core `gateway/session_context.py` (ContextVar design) was already correct — the bug was that WebUI bypassed it. **Option 1 (root fix, `api/streaming.py`):** new `_set_turn_session_identity` / `_reset_turn_session_identity` (+ `_bind_turn_session_identity` context-manager) bind the turn's session id to BOTH `tools.approval._approval_session_key` (resolved first by `get_current_session_key`) and `gateway.session_context._SESSION_KEY`, set in the `_run_agent_streaming` worker thread/context before any agent work and reset (contextvars reset-token semantics) in the same outer `finally` as the existing env restore. The agent worker runs synchronously inside that thread and concurrent tool batches use `contextvars.copy_context()`, so every mid-turn background spawn inherits the binding and the capture is now task/thread-local and race-immune. It deliberately does NOT use the blanket `set_session_vars` (which would zero `HERMES_SESSION_PLATFORM` and break the `notify_on_complete` watcher-registration gate); only the session-key identity is bound, so all other session vars keep their `os.environ` fallback (CLI/cron compatibility preserved). **Option 3 (independent defense-in-depth, `api/background_process.py`):** new `_resolve_wakeup_target` cross-checks the `session_key`-resolved wakeup target against an env-immune spawn owner on the `ProcessSession` (forward-compatible duck-typing of `spawn_session_id`/`owner_session_id`/`turn_session_id`); on a positive mismatch it logs ERROR and re-routes the wakeup to the true owner instead of waking the wrong session. It is a pure pass-through when no env-immune owner is available (today's core, cron/CLI processes sharing the registry, pre-Option-1 spawns), so it never suppresses a legitimate Option Z wakeup on uncertainty. Regression coverage: `tests/test_xsession_wakeup_misroute.py` (RED pre-fix / GREEN post-fix; the concurrent-turns invariant uses the real `get_current_session_key` under a simulated env race). Verified no regression against the 5 wakeup suites + the Option Z/X live-view + RCA resolver + auth-fingerprint suites (85 tests). Core `hermes-agent` is unchanged.

- **Open-tab live-view of a server-initiated wakeup turn was lost when the per-session SSE subscriber was momentarily absent (needed a hard refresh)** (Isla Liu, fork prototype; root-cause follow-up to Option Z/X live-view, on top of `4358d2f`; resolves Copilot PR #2242 review comment #3 in the same machinery). After the Option Z pivot, a server-initiated wakeup turn ran and persisted correctly even with the tab closed (the headline), but a user reported that with a tab OPEN the new prompt + response still did not render live — only a manual browser refresh loaded it. **Root cause, reproduced deterministically with Playwright on the real instance (not speculation — see `liveview-open-tab-fix.md` §1):** the `server_turn_started {session_id, stream_id}` fan-out in `api/routes.py::start_session_turn` is a fire-and-forget `SessionChannel.emit` with **no replay buffer** — it reaches only the subscribers connected at the exact emit instant. A tab whose `/api/session/stream` EventSource is momentarily absent at that instant — a transient SSE drop, a reverse-proxy idle-timeout, or browser connection-pool starvation, all common behind the user's corporate proxy — misses the frame permanently, so the open tab never attaches its renderer to the server-created stream and the wakeup turn appears only after a hard refresh re-reads persisted state. The repro proved the alternative hypotheses false: with a healthy subscriber the warm path renders live (rules out the frontend `attachLiveStream` wiring and a never-emitted frame); server numbers under multi-stream load showed zero SSE-writer thread pileup (rules out the user-suspected "streaming pending" thread starvation); dropping a single subscriber at the emit instant reproduced the loss 100%, and a hard refresh always showed the turn WAS persisted (proves the server-side wakeup itself is unaffected — only live-view was broken). **Fix (minimal, reuses the single renderer — no second one hand-rolled):** `api/background_process.py` adds `active_stream_id_for_session(session_id)` (keys on `ACTIVE_RUNS`, the same registry `_session_has_active_turn` / `_emit_to_session_streams` already trust); `api/routes.py::_handle_session_sse_stream`, immediately AFTER `ch.subscribe()` (so a frame emitted between subscribe and recovery is still queued — no lost-frame gap), looks up any live run for the session and replays a synthetic `server_turn_started {..., source: "subscribe_recovery", recovered: True}` to the freshly-subscribed tab so the open tab self-heals when its EventSource reconnects. `static/messages.js` honours `recovered` by attaching via the reconnecting (replay) path so the renderer rebuilds the in-progress stream from the run journal instead of expecting token 0; a fresh (non-recovered) frame still renders from the first token. Idempotent: the handler's existing `S.activeStreamId === streamId` / `LIVE_STREAMS[sid]` guards make a duplicate (original + recovery) frame a harmless no-op. **Copilot PR #2242 review #3 resolved in the same change** (`_emit_to_session_streams` owner-unknown broadcast fallback): the repro proved the per-session `SessionChannel` — now backed by the on-subscribe recovery — is the SOLE authoritative cross-turn live-view carrier post Option X/Z and the `STREAMS` loop is in-turn defense-in-depth only, so the loop now **skips non-matching AND owner-unknown streams** (`if owner_sid != session_id: continue`, was `if owner_sid and owner_sid != session_id`), removing the cross-session-leak surface Copilot flagged with no live-view regression (verified by the repro + tests). Closed-tab server-side wakeup (the Option Z headline) is strictly unregressed — the recovery is additive and only fires for a session with a live run. Tests: 8 new in `tests/test_optionz_liveview_perf.py` (on-subscribe recovery lookup + behavioural contract + handler/frontend wiring guards; Copilot #3 skip-unknown-owner behaviour, owner-match regression guard, cross-session isolation, source-grep); the headline `test_server_side_wakeup_when_idle_no_tab` + the 37 wakeup-critical tests stay green; full suite 0 regressions vs the post-rebase baseline.

- **Fast-background-task wakeup race — the Option Z defer path had no autonomous-agent drain** (Isla Liu, fork prototype; final follow-up to Option Z, on top of `bdc6e7b`). After the model-resolve hang fix, a `sleep 5` empty bg task self-woke the agent (idle path, Test A ✅) but a `sleep 2` fast task did not (Test B ❌). **Root cause (proven by source, not speculation):** `api/background_process.py::_process_one`'s defer-vs-start branch — when a completion arrives and `_session_has_active_turn(session_id)` is True (`ACTIVE_RUNS` has a row), Option Z cannot start a turn (`start_session_turn` would 409). It only logged and left a bare `PENDING_PROCESS_COMPLETIONS` session flag; the `wakeup_prompt` was **discarded**. The only consumer of that flag was the PR #2279 next-turn drain (`api/streaming._drain_webui_process_notifications`, called inside the turn pipeline at `streaming.py:3445`), which reads `completion_queue` — already emptied by the Option Z drain thread — and is gated by `PROCESS_COMPLETE_EVENTS_SEEN` + registry `_completion_consumed` (both set in `_process_one` *before* the defer). The turn-teardown chokepoint (`streaming.py:4602-4610` `STREAMS.pop` / `unregister_active_run`) has a window between "agent finished output" and "ACTIVE_RUNS cleared" where `_session_has_active_turn` still returns True. A FAST task (2s) completes inside that window → defer → marker left; for an **autonomous agent there is no next user turn**, so the deferred wakeup is lost forever. A SLOW task (5s) completes after teardown finished → idle path → fires. Exactly matches A-success / B-fail. **Fix:** persist the prompt at defer time in `api/config.py::DEFERRED_PROCESS_WAKEUPS` (guarded by `DEFERRED_PROCESS_WAKEUPS_LOCK`) and add a turn-teardown idle-hook `api/background_process.py::drain_deferred_wakeups_for_session` invoked from `api/streaming.py` immediately after `unregister_active_run(stream_id)` (the session has just transitioned active→idle). This makes the busy-at-completion case symmetric with the idle-at-completion case: idle now → fire now (Option Z idle branch); busy now → fire at turn-end when the session goes idle. `claim_deferred_wakeups` pops the per-session entries atomically under the lock, so delivery is exactly-once — idempotent with the next-turn drain (whoever claims first fires; the other finds nothing → no double-fire), no infinite wakeup loop (the wakeup turn's own teardown re-runs the hook and finds nothing claimed), and a multi-stream / cancel-reconnect guard only fires when the just-ended stream was the last `ACTIVE_RUN` for the session. The idle path (Test A), the next-turn drain, and the closed-tab headline are untouched — only the defer path gains the teardown drain; the drain spawns the same throwaway daemon thread `_start_server_side_wakeup_turn` already uses, so streaming teardown never blocks. Tests: new `tests/test_wakeup_defer_race.py` (5 — headline completion-during-teardown, idle-still-once, next-turn+teardown no-double-fire, no-wakeup-loop, multi-stream guard); headline `test_server_side_wakeup_when_idle_no_tab` + the Option Z / AB-coexistence suites stay green; full suite 0 regressions vs the ~5677 baseline.

- **Wakeup turn hang in model-provider resolution — server-side wakeup no longer triggers a blocking live provider-catalog rebuild** (Isla Liu, fork prototype; follow-up to Option Z, on top of `8bad15e`). After the Option Z pivot a server-initiated wakeup turn (`api/background_process._process_one` → `_start_server_side_wakeup_turn` → `api/routes.py::start_session_turn` → `_start_chat_stream_for_session`) visibly never fired for an idle session with the tab closed: no live SSE, refresh showed nothing. **Root cause (proven by a live thread-stack, NOT a race / not compression / not the SSE perf path):** the drain thread fires while the session is idle, so the `/api/models` catalog is cold; `start_session_turn` resolved the model via `_resolve_compatible_session_model_state → get_available_models()`, which cache-missed and ran a **live per-provider rebuild** — `_build_available_models_uncached → _read_live_provider_model_ids → get_copilot_api_token → exchange_copilot_token → urllib HTTPS`. On this WSL/corp network the GitHub Copilot token-exchange endpoint is slow/unreachable and the rebuild blocked the wakeup turn's chat/start at the `resolve_model_provider` stage (captured at 5000ms+ in `bootstrap-8787.log`), so the turn never produced output. Normal human turns rarely hit this because the catalog is usually warm by the time a user interacts, but the drain thread fires exactly when it is cold. **Primary fix** (`api/config.py`, `api/routes.py`): `get_available_models(*, prefer_cache=False)` gains a cache-only mode; `_resolve_compatible_session_model_state(..., prefer_cached_catalog=False)` threads it through; `start_session_turn` resolves with `prefer_cached_catalog=True`. A wakeup now resolves from the warm/disk cache or a network-free minimal catalog (config/auth derived) and trusts the already-persisted `s.model`/`s.model_provider` — the persisted model always wins, so the turn still gets a valid model (persisted → static/default), never a blocking network probe. **Defense-in-depth** (`api/config.py`): the cold live rebuild is now bounded by a wall-clock budget (`_LIVE_REBUILD_BUDGET_SECONDS`, default 4s, `HERMES_WEBUI_MODELS_REBUILD_BUDGET=0` restores the legacy synchronous behaviour). It runs on a daemon worker that signals build-completion via an `Event` (a fast build is never falsely flagged) and publishes its own result into the shared cache; if the budget is exceeded the foreground request returns the best available fallback (last-known disk cache, else the minimal static catalog) immediately and the rebuild finishes out-of-band — removing the synchronous unbounded network dependency from the chat/start hot path that also explains earlier "streaming pending" reports. Least-invasive: no provider-system rewrite, no new dependency, both Option Z/X/live-view paths untouched. Tests: new `tests/test_wakeup_model_resolve_hang.py` (7 — wakeup uses persisted model with no live probe, wiring guard, bounded-rebuild survives a hung probe, minimal catalog is network-free, prefer_cache skips rebuild, two source-grep guards); `test_optionz_liveview_perf.py` / `test_bugbatch_apr2026.py` stubs updated for the new keyword-only param (intentional API change, not a behavioral regression); headline `test_server_side_wakeup_when_idle_no_tab` stays green; full suite 0 regressions vs the ~5677 baseline.

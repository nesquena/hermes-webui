...
- Large session responses now skip the full credential-redaction pass for strings that contain no credential markers, and tail-window session loads no longer send historical `tool_calls` when the returned messages already carry per-message tool metadata. Both changes preserve the existing redaction and tool-card behavior while reducing session-switch latency on long transcripts.

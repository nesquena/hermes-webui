...
- **Custom branding image validation feedback and glow** — Assistant logo and favicon uploads now report the specific failed requirement instead of only repeating the full policy: unsupported type, unreadable image, dimensions below/above the slot limits, timeout, or file size over 12 MB. Custom logo boot also hides the default Hermes fallback while a cached custom logo is being applied, so refreshes show a blank slot briefly instead of a default-logo flash. Custom assistant logos now keep the skin-derived accent glow/background instead of replacing the logo surface with a flat transparent container.

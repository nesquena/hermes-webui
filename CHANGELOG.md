...
- **`process_complete` SSE event — agent wakeup on `terminal(notify_on_complete=true)`** (Isla Liu, fork prototype). Before this change, `terminal(background=true, notify_on_complete=true)` produced no wakeup in WebUI sessions: the background process exited, the agent's `tools.process_registry.completion_queue` queued the completion event, and nothing read from it — the agent only resumed when the user typed something. CLI and gateway adapters (Telegram, Discord, etc.) already drained this queue inside their main loops; WebUI did not. This PR closes the gap with a Path P1 fork-side fix: (a) new `api/background_process.py` drain thread spawned at server startup pulls from `completion_queue`, formats the same `[IMPORTANT: …]` synthetic message CLI/gateway produce, and emits a `process_complete` SSE event onto the matching session's stream. (b) `api/streaming.py` exports `HERMES_SESSION_PLATFORM=webui` and `HERMES_SESSION_CHAT_ID=<sid>` into both the thread-local agent env and the process env — `tools/terminal_tool.py:~1940` gates `pending_watchers.append` on `HERMES_SESSION_PLATFORM`, so without this export `notify_on_complete` events never enqueued (Break B from the spike). (c) `api/routes.py` atomically discards `PENDING_PROCESS_COMPLETIONS` for the session in `_start_chat_stream_for_session`, mirroring the goal_continue marker pattern (#1932). (d) `static/messages.js` subscribes to the new event, shows a small toast, and re-POSTs `wakeup_prompt` to `/api/chat/start` so the agent's next turn sees the completion context. (e) `api/process-complete-ack` endpoint provides a stable diagnostic hook for the frontend. Idempotency: deduped by `(session_id, process_id)` both server-side (`PROCESS_COMPLETE_EVENTS_SEEN`) and client-side (`_seenProcessCompleteIds`). Backward compatible: old WebUI builds graceful-ignore unknown SSE events; clients that never receive the event simply never re-POST and the marker drains harmlessly on the next normal `/api/chat/start`. No DB schema change, no new dependency, no new auth surface — same WebUI session ownership check on `/api/chat/start` as today. Watch-pattern events (`watch_match`, `watch_disabled`) share the same SSE event name with a different payload type, so a single frontend handler covers both. Tests in `tests/test_process_complete_wakeup.py` cover config exports, the drain routing, dedupe semantics, the streaming env-export wiring, and the routes/frontend grep contracts. Manual trial run with isolated `HERMES_HOME` per `AGENTS.md`.

...
if [[ -f "${REPO_ROOT}/.env" ]]; then
  ...
  set -a
  # Equivalent to process-substitution form: source <(grep -vE 'UID|GID|EUID|EGID|PPID' .env)
  # macOS bash 3.2 can fail to import assignments from process substitution,
  # so use a temporary filtered file while preserving the same filter semantics.
  filtered_env="$(mktemp "${TMPDIR:-/tmp}/hermes-env.XXXXXX")"
  grep -vE '^[[:space:]]*(export[[:space:]]+)?(UID|GID|EUID|EGID|PPID)=' "${REPO_ROOT}/.env" > "${filtered_env}"
